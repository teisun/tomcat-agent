...
- **Tomcat Agent Box**：主推 VS Code 二级侧边栏里的对话面板，支持多会话切换、`Chat/Plan` 模式、模型切换、附件、上下文水位，以及产品内的 **Add Models** 设置中心；装好 bundled `.vsix` 就能直接使用。
...
- **VS Code 插件（推荐）**：从 GitHub Release 下载平台对应的 bundled `.vsix`，安装并 Reload VS Code。然后按 `Cmd/Ctrl+Shift+P` 运行 `Tomcat: Focus Agent Box` 打开 **Tomcat Agent Box**；也可以先打开右侧二级侧边栏（Secondary Side Bar）再点击 Tomcat Agent Box 图标。首次使用若看到提示，点击 `Start Setup` 让 VS Code 帮你跑 `tomcat init`。包怎么选、怎么装，见 [`tomcat-vscode-ext/README.zh.md`](tomcat-vscode-ext/README.zh.md)。
- **CLI**：见 **[用户使用说明](tomcat/docs/user-guide.zh.md)**，覆盖前置依赖、构建、`init` / `doctor` / `chat`、会话与工作区、配置、审计及集成测试等完整步骤与示例输出。

通用前提：需要与目标模型匹配的 API 密钥（例如 OpenAI 兼容、DeepSeek、MiMo、GLM、Kimi 或 Anthropic）；无论通过 VS Code 插件还是 CLI 使用，首次运行都可通过 `tomcat init`（或在 VS Code 里点击 `Start Setup`）完成配置，Key 会以 `0600` 权限写入 `~/.tomcat/assets/.env` 并在启动时自动加载。运行态数据默认在 `~/.tomcat/`，目录布局见 [工作目录与数据布局](tomcat/docs/architecture/work-dir-and-data-layout.md)。
...
| 组件 | 作用 | 主要文档 |
| --- | --- | --- |
| [`tomcat/`](tomcat/) | Rust Agent 运行时、CLI 与 `tomcat serve --stdio` | [用户使用说明](tomcat/docs/user-guide.zh.md)、[tomcat/src/README.md](tomcat/src/README.md) |
| [`tomcat-vscode-ext/`](tomcat-vscode-ext/) | VS Code 扩展与 Tomcat Agent Box | [tomcat-vscode-ext/README.zh.md](tomcat-vscode-ext/README.zh.md) |
...
模块级说明见 [tomcat/src/README.md](tomcat/src/README.md)；扩展安装与使用见 [tomcat-vscode-ext/README.zh.md](tomcat-vscode-ext/README.zh.md)。
...
- [tomcat-vscode-ext/README.zh.md](tomcat-vscode-ext/README.zh.md) — VS Code 扩展（Tomcat Agent Box）安装与使用
