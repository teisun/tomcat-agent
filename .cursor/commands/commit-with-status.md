...
- **分支与 status 对应**（与 [Status 规范](../../docs/openspec/specs/guides/workflow/STATUS_GUIDE.md) 一致）：
...
- Status 格式细节见 [STATUS_GUIDE.md](../../docs/openspec/specs/guides/workflow/STATUS_GUIDE.md)（仅用 H3、表格对齐、State 取值、Cov% 列等）。

---

## 3. Commit Message 格式（宪法附录）

必须符合 [Commit Message 规范](../../docs/openspec/specs/guides/workflow/COMMIT_MESSAGE_SPEC.md)：**首行写做了什么（what），详细描述写为什么这么做、作用与意义（why），禁止记流水账**。

- **类型**：`feat` | `fix` | `docs` | `style` | `refactor` | `test` | `chore`
- **豁免（可不写覆盖率）**：以下情况**不需要**填写 `[cov = xx.x%]`：
  - 本次**唯一**修改为 `docs/status/*.md` 或 `docs/INTEGRATION.md`；或
  - **仅修改文档、未改代码**：本次变更仅涉及文档（如 `docs/*.md`、`docs/openspec/**`、`*.md`、guides 等），未修改 `src/`、`Cargo.toml`、测试代码等。  
