...
async function main(): Promise<void> {
  ...
  const harnessTestsPath = path.resolve(
    harnessRoot,
    "out/test/ask-question-reverify.index.js",
  );
  const installRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tmah-"));
  const artifactsRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tmaa-"));
  ...
  const reportPath = path.join(artifactsRoot, "ask-question-reverify-report.json");
  ...
  try {
    ...
    await fs.writeFile(
      ...
      "# Ask question real host reverify workspace\n",
      "utf8",
    );
    await seedReverifySettings(userDataDir, fakeServePath);
    console.log(`Ask question reverify artifacts will be written to: ${artifactsRoot}`);
    ...
    console.log(`Ask question reverify artifacts: ${artifactsRoot}`);
