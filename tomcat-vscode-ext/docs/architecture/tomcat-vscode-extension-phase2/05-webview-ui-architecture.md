...
- request 级空窗期（刚发出请求但还没出现首条 live 内容，或 tool 都完成了但下一段 LLM 输出还没回来）**不再绑回旧组头**，而是统一复用 cluster 尾部的 `ProgressRow`；这行现在不再手写整串 `...`，而是复用共享 [`LoadingDots.tsx`](../../../gui/src/components/LoadingDots.tsx) 原语，以固定三槽位顺序播放更慢、更紧凑、且**首点始终可见**的 `.` → `..` → `...` → `.` 循环，同时保持宽度稳定，不让标题或按钮左右抖动。dots 自己也带 shimmer，所以就算旁边没有可借用的文字，它也能独立表达“系统还在忙”。也就是说：组头只表达“这个块自己还在干活”，而 `ProgressRow` 只表达“整个请求还活着，但此刻没有任何块在工作”；至于 dots 怎么动，则由共享 primitive 一处负责，但**只给没有 shimmer 文本可借用的纯占位 loading 位使用**。因此 `ProgressRow` / `PlanFileCard` 继续共用这套 dots，而 `ThinkingBlock` / `ToolRow` 在 active 态只保留 shimmer 文本，不再叠加第二套 dots 提示。
