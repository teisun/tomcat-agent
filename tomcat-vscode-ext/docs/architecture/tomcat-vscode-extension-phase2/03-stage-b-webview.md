...
| **SB7** 富 UI 控件 | 模型/plan/多会话怎么呈现 | **聊天式 composer + 会话选择栏**：`Chat/Plan` 下拉(`set_plan_mode`) + `Model` 下拉(`set_model`) + `+` 附件 + `Ctx%` + 圆形发送；多会话用 **session 下拉 + new/refresh/close**（`new/switch/close_session`）而非顶部 tab | 本仓 serve `types.rs`(new/switch/close_session 现成)、Stage A `set_plan_mode`/`list_models`；cline `togglePlanActMode`(单 task)、continue `ModeSelect.tsx` | 复用 Stage A 协议；更接近 VS Code Chat composer；多会话仍保留 Tomcat 优势，但在窄侧栏内比 tab 更稳 | 照搬 cline 单活跃 task：放弃 Tomcat 并发多会话能力 → 拒；顶部多 tab：窄侧栏拥挤、与当前会话池/历史恢复 UX 不一致 → 拒 | 底部像 VS Code Chat 一样把模式、模型、附件和发送塞进 composer；顶部只保留精简会话栏。 |
...
| **E-B6** 富 UI 控件 | `Chat/Plan` 下拉 / `Model` 下拉 / 会话选择栏 / 附件 chips / `Ctx%` / 圆形发送 / active-plan strip | `gui/` 组件 + `protocol.ts` → `set_model`/`set_plan_mode`/`new\|switch\|close_session`/`pickAttachment`/`removeAttachment`/`openPlanFile` | `T2B-MULTISESSION-E2E` | 侧栏顶部保留精简会话栏，交互主轴收敛到聊天转录与底部 composer。 |
| **E-B7** 会话池与归属 | webview 启动拉项目 scope 历史(`list_sessions{scope:"disk"}`)填充 session selector、默认选 last-active(`isCurrent`)；激活会话登记 owner，非 owner 只读/提示 | `src/ui/webview/*` + `sessionId→owner(frontend)` 归属表；约定见 [`04`](04-protocol-runtime.md) §6 | `T2B-OWNERSHIP-INT` | webview 列本项目历史、默认接上次那条；同一会话不许俩面板一起发。 |
...
## 3.6 富 UI：会话选择 / transcript / composer

| 控件 | 行为 | 走的协议 | 相对 cline/continue | 说人话 |
|------|------|----------|----------------------|--------|
| 会话选择栏 | 启动列本项目历史 + 默认 last-active；`new/refresh/close` + 下拉切会话 | `list_sessions{scope:"disk"}` → `switch_session` / `new_session` / `close_session` | 保留 Tomcat 多会话能力，但在窄侧栏比 tab 更稳 | 顶部只留一个精简会话栏，切历史和新建都在这里。 |
| transcript 时间线 | 顺序渲染消息 / thinking / tool / approval / plan 文件卡片，并把历史与实时事件合并 | `get_messages` + `event` 透传 + `state` 快照 | 比继续堆原生 participant 更接近完整 chat transcript | 聊天记录不再分块散落，而是像真正聊天窗口一样往下排。 |
| `Chat/Plan` 下拉 + active-plan strip | 下拉只保留 `Chat` / `Plan`；`Plan`=enter，`Chat`=exit；`Build` 独立放在 planning/executing/pending 时出现的计划条上 | `set_plan_mode` + `plan.*` + `get_state.planState` | 比 cline Plan/Act 更细（5 态）且更贴近用户指定 UX | 进计划靠下拉，执行计划靠上方细条里的 `Build`。 |
| 模型下拉 | 列模型 + 切换 + 标当前 | `list_models`→`set_model`→`get_state.model`（Stage A） | 同 continue 模型选择 | 在 composer 底栏切模型。 |
| 附件 chips + 文件选择 | `+` 触发原生 `showOpenDialog`，文件转 base64 挂到待发送列表；chips 显示在输入框上方，可移除 | `pickAttachment` / `removeAttachment` / `prompt.attachments[]` | 比单纯文本输入更接近 VS Code Chat / Continue 的多模态入口 | 点 `+` 选文件，先挂 chip，再跟消息一起发。 |
| `Ctx%` + 圆形发送 | 展示 `context_metrics_update.contextUtilizationRatio`；发送按钮常驻右下 | `context_metrics_update` + `prompt` | 更接近 VS Code Chat composer 的密集底栏 | 用户一眼就知道上下文用了多少，还能点圆形按钮发消息。 |

> 说人话：多会话仍然是 Tomcat 的独门优势——serve `registry` 本就并发持有多会话（Phase 1 已用），只是最终 UI 没选顶部 tab，而是选了**更适合窄侧栏的 session 选择栏**。Stage B 在此之上继续复刻 `tomcat code` 的"按 git 项目根归组 + 默认恢复 last-active"：webview 启动调 `list_sessions{scope:"disk"}` 拉同一项目 scope 历史（与 `tomcat code` 共写一份 `sessions.json`），默认选中 `isCurrent` 那条。
...
### 3.6.2 最终落地 UI 结构（聊天式 webview）

> 专业：最终实现不是“顶部一排大按钮 + 分块面板”，而是 **transcript-first** 的聊天式 webview。状态层改为 timeline 模型，把历史补齐（`get_messages`）与 live `WireEvent` 合并后统一渲染；交互重心收敛到底部 composer 与条件显示的 active-plan strip。
> 说人话：整个侧栏现在更像 VS Code `CHAT` 本身，而不是一个把按钮和卡片东一块西一块堆起来的控制台。

```text
Header / SessionBar
  session select + New / Refresh / Close

Transcript
  user / assistant / thinking / tool / approval / plan-file-card

ActivePlanStrip (only planning / executing / pending)
  plan file + plan state + Build

AttachmentChips (only when pending attachments exist)
  removable chips rendered above composer

Pinned Composer
  textarea
  [ + ] [Chat/Plan] [Model] [Ctx N%] [Send]
```

落地到组件与职责：

- `SessionBar`：顶部精简会话栏，用下拉列出共享项目 scope 会话池；提供 `New / Refresh / Close`。
- `TranscriptView`：渲染统一 timeline，顺序承载消息、thinking、tool、approval、plan 文件卡片。
- `ActivePlanStrip`：仅 `planning / executing / pending` 显示；`Build` 放此处，`executing` 时禁用。
...
- 附件链路为：`+` → `pickAttachment` → `showOpenDialog` → 读取文件/base64 → `pendingAttachments[]` → chips → `prompt.attachments[]`。
- `Ctx%` 来自 `context_metrics_update.contextUtilizationRatio`；因此用户在真实发送消息后能看到预算水位立即变化。
