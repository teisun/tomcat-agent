...
### 8.1 Add Model 双模式手测脚本（devhost）

前置条件：先跑过一次 `tomcat init`，再启动扩展 devhost，打开 `Tomcat: Open Settings`，进入 `Models` 页。下面每一步都要求观察 UI、保存后的模型列表，以及再次打开编辑弹窗时的回填结果。

1. 官方 Tab 默认可用 + Provider 非空
   预期：点击 `Add Model` 后默认落在 `Official new model` Tab；`Provider` 下拉里至少出现一条官方预置（例如 `OpenAI`/`Anthropic`/`DeepSeek`），且下拉可以正常展开选择，不应出现空白、点不开或只有 0 个选项的情况。
2. 模式 A｜官方新增模型 + 首次 Key
   预期：保持在 `Official new model` Tab，选一个官方 provider preset，填写 `Model name` 与首次 `API key` 后可以保存；保存后模型出现在模型选择器里，`id` 默认等于 `model_name`，`api/base_url/api_key_env/thinking_format/capabilities` 都来自 preset。
3. 模式 B｜中转站派生链路
   预期：切到 `Relay / custom endpoint`，填写 `base_url + model_name + API key` 后，实时派生预览立即显示正确的 `provider / env / id`；例如 `https://api.chatanywhere.tech/v1 + gpt-5.4` 应显示 `chatanywhere / CHATANYWHERE_API_KEY / chatanywhere/gpt-5.4`。
4. 无官方预置时自动回落到中转站 Tab
   预期：若连接到一个只返回 `source=user` 模型的 serve，点击 `Add Model` 后默认落在 `Relay / custom endpoint` Tab；若手动切到 `Official new model`，界面应显示「当前没有官方预置，请改用中转站/自定义端点」之类的引导，不应把用户卡在一个无字段、无原因的死路里。
5. 编辑回填模式判定
   预期：保存一条 relay user 模型后重新编辑，弹窗应直接落在模式 B；`base_url`、`API`、`model_name`、派生预览与 key slot 都按已保存值回填，不应错回模式 A。
6. 内置 id 防撞提示
   预期：把 user 模型的 `id` 改成与 builtin 同名时，界面提示「将覆盖内置模型 X」；该提示只警告、不阻断保存，保存后覆盖行为与既有后端语义一致。
7. Advanced 覆盖 thinking format
   预期：在模式 B 打开 `Advanced`，把 `Thinking format` 从 `Auto` 改成 `deepseek` 后保存；随后触发一次请求时，后端应按 `deepseek` 方言编码，而不是继续按 wire 自动值发送。
8. 非法 base_url 负路径
   预期：输入不可解析的 `base_url`（如只填 `https://`）时，保存按钮不能通过校验；界面显示方向性错误文案 `Base URL could not be parsed. Use a value like https://host/v1.`，引导用户修正而不是给出模糊失败。
