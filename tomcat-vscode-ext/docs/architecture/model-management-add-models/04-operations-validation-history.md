...
| 变量 | 现状 | 本次 | 覆盖 |
| --- | --- | --- | --- |
| `LITELLM_SUNMI_API_KEY` | 已有 | 不动 | OpenAI wire（SUNMI 中转，`openai-responses`）|
| `DEEPSEEK_API_KEY` / `MIMO_API_KEY` | 已有 | 不动 | `deepseek-v4-*` / `mimo-v2.5-pro` |
| `ZHIPU_API_KEY` | 缺 | GLM 官方 key | `glm-5.2`（path-aware `/api/paas/v4`）|
| `MOONSHOT_API_KEY` | 缺 | Kimi 官方 key | `kimi-k2.7-code` |
| `ANTHROPIC_API_KEY` | 缺 | Anthropic 中转站 key | `claude-opus-4-8/4-7/4-6`（新 `anthropic-messages`）|

- Anthropic 中转站 base_url ≠ `api.anthropic.com`：集成测试加 `TOMCAT_E2E_ANTHROPIC_BASE_URL`（可选 `TOMCAT_E2E_ANTHROPIC_MODEL`），仿 `MIMO_TEST_BASE_URL_ENV`；运行时在 `models.toml` 覆盖 claude 模型的 `base_url`。
- OpenAI 官方口不测，用 SUNMI 覆盖 openai wire；未填 Key 的官方内置保持「Needs API key」，当门控样本。
...
| real-LLM 冒烟  | env 门控可跳过：GLM/Kimi 用官方 Key、Anthropic 用中转站 Key + `TOMCAT_E2E_ANTHROPIC_BASE_URL` 覆盖、OpenAI 走 SUNMI（凭证见 §6.1）                                          | PENDING          | 真机连通冒烟。            |
