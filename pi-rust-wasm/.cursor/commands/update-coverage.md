...
| 分支 | status 文件 |
|------|------------|
| `feature/xxx` | `status/feature-xxx.md` |
| `develop` | `status/develop.md` |
| 其他 | `status/<分支名中 / 替换为 ->.md` |
...
在项目根目录（`pi-rust-wasm/`）执行：

```bash
cargo tarpaulin --lib --package pi_wasm
```
...
```
覆盖率已更新：XX.XX%
status 文件已同步：status/feature-xxx.md
