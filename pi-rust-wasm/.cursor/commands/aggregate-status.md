...
2. **收集碎片**  
   对以下各分支执行（分支名 → status 文件名见 status/README.md）：
   - `feature/infra` → `git show feature/infra:status/feature-infra.md`
   - `feature/session-cli` → `git show feature/session-cli:status/feature-session-cli.md`
   - `feature/llm` → `git show feature/llm:status/feature-llm.md`
   - `feature/wasm-plugin` → `git show feature/wasm-plugin:status/feature-wasm-plugin.md`
   - `feature/primitives-tools` → `git show feature/primitives-tools:status/feature-primitives-tools.md`
   - `feature/chat` → `git show feature/chat:status/feature-chat.md`  
   若某分支无该文件或读取失败，该节内容记为「（暂无进度碎片）」。
