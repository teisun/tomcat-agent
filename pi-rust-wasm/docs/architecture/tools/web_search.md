...
| **本仓库 `web_search`**（路线图） | `trait WebSearchBackend` + 4 适配器 | `backend=auto`：**仅** `[llm] provider = "openai-responses"` → server-side；`"openai"`（Completions）与其余 provider → Tavily→Brave→Serper 降级链 | `query` + `count` + `freshness` + `country` + `language` + `domain_filter` | hits.url 归一化阶段 **pi 增补** SSRF + 限速 → `truncated+warning` | managed schema 对齐 openclaw；**native 门闩对齐本仓双 OpenAI provider 事实**（见 [`registry.rs` L47-50](../../../src/core/llm/registry.rs)） |
...
- **配置承接**：`ToolsWebSearchConfig { backend, api_key, count, freshness, country, language, domain_filter, cache_ttl_secs, cache_capacity, timeout_ms }` 加到 [`infra/config/types.rs`](../../../src/infra/config/types.rs)；env 优先级 `PI_WASM__TOOLS__WEB_SEARCH__*`。
...
| 来源 | 键 | 含义 | 备注 | 说人话 |
|------|-----|------|------|--------|
| `pi.config.toml` | `[tools.web_search] backend` | `auto` / `tavily` / `brave` / `serper` / `openai` | 默认 `auto` | 不写就跟模型走。 |
| `pi.config.toml` | `[tools.web_search] api_key` | 单家 backend 的 key（与 backend 字段对应） | 推荐用 env 覆盖 | 别写文件，写环境变量。 |
| env | `PI_WASM__TOOLS__WEB_SEARCH__BACKEND` | 同上，运行时覆盖 | env > config | 容器里临时换 backend。 |
| env | `PI_WASM__TOOLS__WEB_SEARCH__API_KEY` | 同上 key | 与 backend 字段配对 | 部署面板里的 secret。 |
...
| `pi.config.toml` | `[tools.web_search] count` | 默认期望 hits 数 | 1..=20，默认 5 | 一次默认搜几条。 |
| `pi.config.toml` | `[tools.web_search] freshness` / `country` / `language` | 缺省筛选 | 入参可覆盖 | 配置层定个默认；模型可改。 |
| `pi.config.toml` | `[tools.web_search] domain_filter` | 默认白名单 | 入参可叠加 | 「这台机只搜这几个站」。 |
| `pi.config.toml` | `[tools.web_search] blocked_domains` | 黑名单 | 与 `domain_filter` 互补；与 `web_fetch.allowed_domains` **不共享** | 拒搜某些站。 |
| `pi.config.toml` | `[tools.web_search] cache_ttl_secs` / `cache_capacity` | LRU TTL 与容量 | 默认 300 / 50 | 缓存活多久、能装多少。 |
| `pi.config.toml` | `[tools.web_search] timeout_ms` | 单次 backend 超时 | 默认 12_000 | 等多久还没回来就算超时。 |
