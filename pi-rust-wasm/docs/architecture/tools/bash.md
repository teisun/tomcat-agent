...
- **说人话（把上条里的术语拆开）**：
  ...
  - **超字符上限写 `persisted_output_path`**：当合并后的输出**超过配置里的字符上限**（规划键 **`[tools.bash].max_output_chars`**，见 §8「规划」表，**默认/上限度数以 PR-E 钉死为准，当前文档仍为 PENDING**）时，把**完整原文**写到磁盘（`~/.pi_/…/tool-results/…`），并在返回 JSON 里给出 **`persisted_output_path`**，模型可按路径自己去读尾部。**另一套独立限制是墙钟 `timeout_ms`**：默认 **120_000 ms**、配置上限 **600_000 ms**——管「跑多久」，不管「输出多长」；两条线同时生效。
...
```jsonc
{
  "command": "cargo test -p pi_wasm --lib",
  "cwd": "/abs/path/to/pi-rust-wasm"
}
```
...
| 来源 | 键 | 含义 | 备注 | 说人话 |
|------|-----|------|------|--------|
| `pi.config.toml` / `PrimitiveConfig` | `wasmedge_env_path` | Unix 下 `sh -c` 前可选 `source` 的脚本 | 默认 `$HOME/.wasmedge/env`；见 [`infra/config/types.rs`](../../../src/infra/config/types.rs) 注释 | WasmEdge 环境注入用。 |
