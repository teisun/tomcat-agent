...
pi 默认将所有数据存放在 `~/.pi_/`。可在 `pi.config.toml` 的 `[storage]` 段或通过环境变量 `PI_WASM__STORAGE__WORK_DIR` 覆盖：
...
```bash
pi doctor --config /tmp/my-pi/pi.config.toml
```
...
`pi config` 提供对 `pi.config.toml` 的读写、导入导出操作，无需手动编辑文件。
...
需要在 `pi.config.toml` 中开启审计并（可选）设置保留天数：
...
> 所有 `PI_WASM__<SECTION>__<FIELD>` 变量均可覆盖 `pi.config.toml` 中对应字段，`__` 作为嵌套分隔符。
