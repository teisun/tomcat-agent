...
- [✓] **[P0]** 分支侧门禁：`cargo fmt --all`、`cargo clippy --all-targets -- -D warnings`、`cargo test --lib`（826 passed）、`cargo test --test cli_tests -- --test-threads=1`（77 passed）、`RUST_LOG=pi_wasm=debug,info ./scripts/run-integration-tests.sh all`（全绿）。
