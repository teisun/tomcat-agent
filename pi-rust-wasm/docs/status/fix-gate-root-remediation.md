...
- `GateConfig` / `DefaultPermissionGate`：默认可写根为 `agent_definition_dir`；启动 `cwd` / `agent_workspace_dir` 仅作 prompt 语义，访问需 `workspace_roots`、会话授权、拖拽或 cwd lazy。
...
- `DefaultPrimitiveExecutor::new(..., gate: Arc<dyn PermissionGate>)` 必填；无 `with_gate` / `with_workspace_roots` builder。
