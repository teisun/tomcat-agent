...
### ✅ DONE (已完成)
- [x] **[P0]** 20.1 ~~TurnEntry~~ 增加 `id: String`；ContextState 增加 `transcript_path`/`compaction_summary` [已重构: `TurnEntry` 删除，id 改为 `ChatMessage.msg_id`]
...
- [x] **[P1]** 20.5 Transcript **单行 compaction 不变式**：预热追加一行 `is_boundary=false`（含行 `id` / 可选 `preheatCompactionId`）；应用时 **原地** 将该行 `isBoundary` 改为 `true`（不追加第二份全文）。`init_context_state` 在 `fold_entries_to_messages` 单遍推断最后一条未应用 preheat 时调用 `preheat.restore_completed`，重载后与运行时一致（§5.5′） [已重构: `fold_entries_to_turns` → `fold_entries_to_messages`]
...
- [x] **TASK-21 §5.7 消息级 ID**：~~`UserTurn` 增加 `start_id`/`end_id` 与 `compound_turn_id`~~ [已重构: `UserTurn` 删除，改为 `ChatMessage.msg_id`]；`append_message`/`try_append_message` 返回新 message 行 id；`chat` 先写满 transcript 再 `messages.push` [已重构: 原 `on_new_user_turn` 删除]；`fold_entries_to_messages` [已重构: 原 `fold_entries_to_turns`] 从 message id 还原；`insert_entry_after_message_id`（锚点缺失 warn + `append_entry`）；Preheat 快照 `S`/`E`、`BranchSummaryEntry.id = S::E`；`apply_boundary` 以 `msg_id` 为主 + 旧 id 回退；`restore_completed` 与 `msg_id`/suffix 匹配
...
- ~~`compound_turn_id` / `compound_id_prefix` / `compound_id_suffix`~~：[已重构: 删除，不再需要复合 id]
...
- `apply_boundary`：仅按 **`covered_end_id`** 在 `messages` 中 `rposition` 定位匹配 `msg_id` 的消息 [已重构: 原在 `user_turns_list` 中定位]；无匹配 → **`AppError::ApplyBoundaryStale`**，Layer 2 删对应 **`branch_summary`** 行且不 **`restore_pending_result`**（见 `context-management.md` §5.7.5.1）。
- **不向前兼容**：开发阶段不实现「同一逻辑批次两条全文 compaction（false 行 + 再 append true 行）」；历史 JSONL 需手工整理或新 session。
- ~~`TurnEntry::UserTurn`/`SummaryTurn`~~: [已重构: 删除，改为 `ChatMessage` + `MessageKind`，id 改为 `ChatMessage.msg_id`]
