...
- [✓] **[P1]** 5.1 ~~AgentMessage 枚举与 convert_to_llm_format()~~ [已重构: 删除，统一使用 ChatMessage + MessageKind]
...
- ~~`AgentMessage` 枚举~~ [已重构: 删除，统一使用 `ChatMessage` + `MessageKind`（Normal/Steering/CompactionSummary）]
- ~~`convert_to_llm_format(messages)`~~ [已重构: 删除，`ChatMessage` 直接发给 LLM]
- ~~`agent_messages_from_chat(messages)`~~ [已重构: 删除，不再需要反向转换]
