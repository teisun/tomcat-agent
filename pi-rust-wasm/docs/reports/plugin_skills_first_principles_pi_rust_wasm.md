# 插件系统与 Skills：第一性原理报告（pi-rust-wasm）

**版本**：1.1（**v1.1**：新增附录「`@openclaw/plugin-sdk` 子路径一览」，与上游 `main` 的 `exports` 对齐）  
**日期**：2026-04-19  
**落盘路径**：`pi-rust-wasm/docs/reports/plugin_skills_first_principles_pi_rust_wasm.md`  
**范围**：从第一性原理回答「是否必须做插件系统」、与四款 Agent 项目对照、对 **pi-rust-wasm** 的分阶段建议；**不**替代既有实现细节报告（如 [plugin_systems_openclaw_pi_mono_pi_agent_rust.md](plugin_systems_openclaw_pi_mono_pi_agent_rust.md)）。
...
- **结论**：对 pi-rust-wasm 而言，**完整「插件市场」式体系不是 MVP 必要条件**；应先稳住 **会话、工具、权限、流式、记录**，再以 **Skills 等价物 → 窄契约扩展（声明式 / Wasm + hostcall）→ 长生命周期兼容层** 分层推进。
- **常见误区**：Hermes **并非**「只有 Skills」——仓库内同时存在 **Python 插件**（manifest + `register` + hooks）与 **Skills**（`SKILL.md`）；二者双轨并存。
- **与本仓库 TODO 对齐**：已将插件/VM 簇若干条目从 **P1 调整为 P2**（见 [§9](#9-与-pi-rust-wasmdocstodosmd-的同步修订)），避免与核心体验类 P1 混淆心理压力。
...
因此：**「只有 Skills、没有插件」不成立**；对比 OpenClaw / pi 系时应区分「文档技能轨」与「可执行扩展轨」，Hermes 是 **双轨** 产品。
...
对 **pi-rust-wasm** 的启示：**不必**在一条线上同时复制「OpenClaw 插件 + pi-mono 扩展 + pi_agent_rust WASM」的全部面积；应按 **目标风险与兼容需求** 选层。
...
## 5. 对 pi-rust-wasm 的分阶段建议（忽略当前实现细节）
...
- **优先**：**宿主内建工具**完善 + **声明式策略**（权限、目录、hostcall 分类）；若需用户逻辑，倾向 **Wasm + 窄 ABI + hostcall 闸门**（与 pi-rust-wasm 已有 Wasm/VM 方向一致）。
...
| Provider 流式 | `provider-stream-shared` | Pi 流包装与 `tool_stream` 参数合成。 |
...
## 9. 与 `pi-rust-wasm/docs/TODOS.md` 的同步修订
