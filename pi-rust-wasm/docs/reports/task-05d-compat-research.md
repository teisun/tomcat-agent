...
> 2. pi-mono / pi_agent_rust / pi-rust-wasm 三者的 UI 渲染架构分别是什么？
> 3. pi-rust-wasm 应采用什么方案处理 npm 包 import？与 pi_agent_rust 的做法有何异同？
...
这种架构**不可直接移植**到 pi-rust-wasm，因为：

1. pi-rust-wasm 的扩展跑在 Wasm 沙箱内，无法访问 `process.stdout` / `process.stdin`
...
## Part 4: 方案 C 详解 — pi-rust-wasm 推荐方案（编译时 import 重写 + globalThis 注入）

### 问题

pi-rust-wasm 使用 **WasmEdge QuickJS**（编译为 Wasm 运行在沙箱中），采用**脚本模式**——所有 JS 拼接为一个字符串通过 `eval()` 执行。这意味着：
...
```
pi-rust-wasm 方案 C 架构（编译时 import 重写）
================================================
...
### pi_agent_rust vs pi-rust-wasm 方案 C 对比

| 维度 | pi_agent_rust | pi-rust-wasm 方案 C |
|---|---|---|
...
- pi-rust-wasm 只能在**编译时重写 import**（因为 WasmEdge QuickJS 用脚本模式执行，无模块加载回调），所以需要 SWC 把 import 改成 globalThis 引用

### 推荐策略

**效仿 pi_agent_rust 的理念（为 npm 包提供 shim 降级实现），适配 pi-rust-wasm 自身引擎约束（SWC 编译时 import 重写 + globalThis 注入）。**
