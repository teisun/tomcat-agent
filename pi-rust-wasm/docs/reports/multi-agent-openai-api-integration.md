...
**文档性质**：基于 Tomcat 工作区内仓库的代码检索与模块阅读整理的技术报告，供 `pi-rust-wasm` 选型与对标参考。  
...
| **pi-rust-wasm** | 完整 | `src/core/llm/*` 当前唯一 OpenAI 实现为 Chat Completions |
...
## 7. pi-rust-wasm（本项目）：薄 trait + 单一 OpenAI Chat Completions
...
### 7.3 ASCII：pi-rust-wasm 当前调用栈（单路径）
...
### 7.4 ASCII：pi-rust-wasm 全链路（入口 → Agent → LLM HTTP）
...
| 维度 | pi_agent_rust | hermes-agent | openclaw | pi-mono | pi-rust-wasm（现状） |
|------|----------------|--------------|----------|---------|----------------------|
...
```text
pi_agent_rust   hermes-agent      openclaw              pi-mono              pi-rust-wasm（现状）
─────────────   ────────────      ────────              ───────              ─────────────────────
...
## 9. 对 pi-rust-wasm 的启示（非规范，仅技术建议）
...
**结论先说**：**可以两者并存**——**实现类型（哪一家 API）** 与 **模型名字符串** 常常分开；**pi-rust-wasm 当前是「代码定实现 + 配置定模型名」**。
...
- **pi-rust-wasm**：**实现类固定**（当前仅 OpenAI 适配器），**参数靠配置**；要「配置切换 Anthropic」需 **产品化改造**（配置里增加 `provider` + **工厂**或 **match**）。  
...
你的想法与 **「同一 `LlmProvider`，不同调用路径选不同 `model` 字符串」** 完全一致；这在 pi-rust-wasm 里 **不与 §9.2 冲突**——换厂商仍是 trait/工厂问题，**换场景仍是模型 ID + 请求体差异**。
...
- **pi-rust-wasm**：`pi-rust-wasm/src/core/llm/provider.rs`，`pi-rust-wasm/src/core/llm/openai.rs`，`pi-rust-wasm/src/core/llm/types.rs`；**Provider 构造**：`pi-rust-wasm/src/api/chat/mod.rs`（`ChatContext::from_config` 内 `OpenAiProvider::new`）；**主对话模型**：同文件 **`effective_model`**；**压缩摘要模型**：`pi-rust-wasm/src/infra/config/types.rs` **`ContextConfig.compaction_model`** + `pi-rust-wasm/src/core/compaction/preheat.rs` **`generate_summary`**
...
| 2026-05-04 | 新增 **§7.4**：pi-rust-wasm 全链路 ASCII（入口 → AgentLoop → `LlmProvider` → `/v1/chat/completions`） |
| 2026-05-04 | §9.1 增补 **（E）（F）**：pi-rust-wasm 演进两条岔路 + 每接一家 LLM 的清单与 ASCII |
