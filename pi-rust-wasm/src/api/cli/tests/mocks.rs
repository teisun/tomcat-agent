//! # CLI 测试共享 fixture
//!
//! - `with_pi_config_in_home`：把进程 `HOME` 指向临时目录并预先写入
//!   `~/.pi_/pi.config.toml`，以便 `pi workspace` 等子命令读写真实配置文件
//!   时不影响开发机 `~/.pi_`。多个工作区相关用例共用一把全局锁串行化执行。
...
pub(super) fn with_pi_config_in_home<R>(work_dir: &std::path::Path, f: impl FnOnce() -> R) -> R {
