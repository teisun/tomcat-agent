...
//! - `openai_provider`：`OpenAiProvider::new` / `count_tokens` / `is_retriable`
//!   等不依赖网络的可执行行为；以及 `chat_real_request_response_print`
//!   这种依赖真实 OPENAI_API_KEY 的 `#[ignore]` 用例。
//! - `openai_stream`：SSE 流解析与 `OpenAiStreamChunk → StreamEvent` 的转换。
//! - `token_usage`：`SessionTokenUsage::add` 累加正确性。
//! - `types`：`ChatMessage` / `ChatRequest` / `StreamEvent` 的构造与序列化。
