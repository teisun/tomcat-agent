...
/// 内置默认 path_rules。
///
/// 涉及凭据保护（`~/.ssh` / `~/.aws` 等）+ pi 自身配置/凭据保护（`~/.pi_/...`）
/// + Agent 数据目录可读不可写（`~/.pi_/agents/*/{sessions,logs,audit}`）。
pub const BUILTIN_DEFAULT_PATH_RULES: &[(&str, PathRuleMode)] = &[
    ...
    // ── pi 自身配置/凭据保护 ──
    ("~/.pi_/pi.config.toml", PathRuleMode::Deny),
    ("~/.pi_/credentials", PathRuleMode::Deny),
    ...
    (
        "~/.pi_/agents/*/agent/auth-profiles*.json",
        PathRuleMode::Deny,
    ),
    ("~/.pi_/agents/*/agent/credentials", PathRuleMode::Deny),
    ("~/.pi_/agents/*/agent/credentials/**", PathRuleMode::Deny),
    // ── Agent 历史与审计：可读不可写 ──
    ("~/.pi_/agents/*/sessions", PathRuleMode::Readonly),
    ("~/.pi_/agents/*/sessions/**", PathRuleMode::Readonly),
    ("~/.pi_/agents/*/logs", PathRuleMode::Readonly),
    ("~/.pi_/agents/*/logs/**", PathRuleMode::Readonly),
    ("~/.pi_/agents/*/audit", PathRuleMode::Readonly),
    ("~/.pi_/agents/*/audit/**", PathRuleMode::Readonly),
];
...
/// 3. **Agent 自我提权防护**（pi config set、>~/.pi_/...、sed -i ~/.pi_/）
pub const BUILTIN_BASH_FORBIDDEN: &[&str] = &[
    ...
    r#"^\s*pi\s+config\s+set\b"#,
    r#"^\s*pi\s+config\s+edit\b"#,
    r#"^\s*pi\s+pathrules\s+(add|remove|clear-session)\b"#,
    r#"^\s*pi\s+workspace\s+(add|remove)\b"#,
    r#">\s*~/\.pi_/pi\.config\.toml"#,
    r#">\s*~/\.pi_/credentials"#,
    r#"^\s*(sed|awk|perl)\b.*-i.*~/\.pi_/"#,
];
