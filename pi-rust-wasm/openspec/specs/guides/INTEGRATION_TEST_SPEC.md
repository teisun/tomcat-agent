...
### 5.2 Mocking 策略
*   **外部 API**：推荐使用 `wiremock` 或 `mockito` 模拟 HTTP 服务。
*   **代码级 Mock**：对于难以集成的逻辑，使用 `mockall` 宏生成 Mock 对象。
...
---
在集成测试中，日志（Logging）和链路追踪（Tracing）至关重要。由于 Rust 的测试默认会捕获并隐藏标准输出，如果没有合理的日志规范，当 CI（持续集成）环境下的测试失败时，开发者将很难定位具体的失败原因。

以下是针对 **日志与追踪（Logging & Tracing）** 补充的专项规范，建议将其作为第 9 章节加入原文档：
