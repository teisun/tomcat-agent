...
## 1. 目标与定位
本规范旨在统一 `pi_wasm` 项目的模块文档格式。一份合格的文档应具备：
- **可操作性**：开发者看后能立即知道如何对接 API。
- **架构清晰**：阐明模块在全局中的位置及依赖关系。
- **防御性设计说明**：记录并发、错误处理、安全等关键设计决策。

### 1.1 落盘位置（与 `src/` 对照的模块说明）

- **目录**：与 `src/` 子模块一一对应的技术说明，统一放在仓库根目录下的 [`docs/technical/`](../../../../docs/technical/README.md)（索引与编号规则见该目录下 `README.md`）。
- **边界**：用户指南（`docs/user-guide.md`）、组内分享（`docs/sharing/`）、进度碎片（`docs/status/`）**不替代** `technical/` 中的模块文档；代码变更若影响对外行为或模块边界，应同步更新对应的 `docs/technical/*.md`。
...
- **核心文件**：模块对应的核心代码路径。
- **建议**：在概述后增加 **ASCII 架构或数据流图**（标出本模块在栈中的位置及与邻模块的边界），并与 [`docs/technical/README.md`](../../../../docs/technical/README.md) 中的总览图对照，避免重复画整张系统图。
...
3. **图文并茂**：概述优先用 **ASCII 图**（易 diff、易在终端阅读）；复杂时序或类关系可补充 **Mermaid**。模块文档的图示分工见 `docs/technical/README.md`。
