...
- **真实性优先**：核心 LLM 逻辑优先真实接口调用；**外部成熟 API（OpenAI 等）不 mock、用真实请求**；自测时若主 API 与配置的 fallback 均不可用，**再使用 mock 数据**完成该用例，保证单测可稳定通过（优先级：真实请求 > 降级重试 > mock）。**内部未完成模块**因开发进度可用 mock。
...
- 提交前须跑通约定测试并算出覆盖率；完成定义要求“单元测试通过，算出覆盖率”。
