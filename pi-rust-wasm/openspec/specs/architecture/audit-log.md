...
本文档描述 pi-rust-wasm 独立审计日志模块的技术方案，与基础设施层「日志与审计」对应，实现 T1-P1-001 审计日志系统完整落地。
...
CLI `pi audit list` 执行前可选调用 `store.cleanup()`，按配置的保留天数清理。

## 7. CLI 对接

- `pi audit list [--limit N]`：使用 `AuditStore::query`，输出序号、时间、类型、状态、详情。
- `pi audit show <id>`：按 id（行号）从 query 结果中取单条展示。
- `pi audit export <path>`：调用 `AuditStore::export_to`。
