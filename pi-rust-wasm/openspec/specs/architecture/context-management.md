...
- **`apply_boundary` 失败**：发出 **`CompactionError`**（`source: "apply"`、`exhausted_after_retries: false`），并 **`restore_pending_result`** 以便后续重试 apply。
...
| `[src/core/session/transcript.rs](../../../src/core/session/transcript.rs)` | `BranchSummaryEntry` 可选 `preheatCompactionId`；`set_branch_summary_entry_is_boundary_true` 按 id 原地升级                                                     |
...
- `compaction_error`：`source: "preheat"` 且 **`exhausted_after_retries == true`** → **待恢复**提示（需结合 ⑤/② 的 `try_restart_if_pending` 或用户操作）；`source: "apply"` → apply 失败、摘要仍待重试
