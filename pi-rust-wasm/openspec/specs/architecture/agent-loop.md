...
2. **pi 生态兼容**：事件名、Steering/FollowUp 语义、消息类型边界与 pi-mono 一致。
...
1. **基础身份**（固定）：如「你是一个 AI 编程助手，运行在 pi-rust-wasm 环境中…」
...
│  ├─ tool_execution_start  ← 每个工具执行前
│  ├─ [tool_execution_update] ← 工具执行中（可选，如 bash 流式）
│  ├─ tool_execution_end    ← 每个工具执行后
│  └─ turn_end           ← 本轮结束
...
- **tool_call**（见 [events.md](events.md)）：工具调用前，可观察参数。
- **tool_result**（见 [events.md](events.md)）：工具执行后，可观察/变换结果。
