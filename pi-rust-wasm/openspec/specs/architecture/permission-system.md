...
- 用户 / builtin 都可能写 deny，**任何 Allow 来源都不能弱化它**。如果放在 Layer 2 之后，agent_data_readonly_dirs 兜底会让 `~/.pi_/credentials` 变成可读，违反最小权限。
...
- `SessionGrants` / `DraggedPaths` 是会话内可变的两个 `Arc<Mutex<…>>`，由 `ChatContext` 持有同一份并注入 gate；UI 侧追加授权后，gate 与 system prompt 在下次调用即看到。
- `defaults.rs` 提供两组 `&'static [_]` 常量，gate 在 `effective_path_rules()` 时与 user 规则合并。
...
```text
   ...
   行内含意图         纯路径
       │                │
       ▼                ▼
   AllowOnce       gate.check(Read, path)  (探针)
   (无菜单，                │
    立即追加         ┌──────┼──────┐
    dragged_paths    │      │      │
    并执行)         deny  ronly  其它
                     │      │      │
                     ▼      ▼      ▼
                 deny_only  ronly_only  full
                 [d][c]    [r][d][c]    [a][r][w][d][c]
                                            │
                  ┌─────────────────────────┼──────────────────────┐
                  ▼                         ▼                      ▼
              [a] AllowOnce          [r] ReadOnlyOnce       [w] AllowAndPersistRoot
              SessionGrants.push     SessionGrants.push     append_extra_root_to_disk
              (write=true)           (write=false)          (写盘 + filelock)
                                                                   │
                                                                   ├── [d] append path_rule deny (写盘)
                                                                   └── [c] cancel (无副作用)
```

行内意图的判定见 `src/api/chat/dragged_path.rs::looks_like_intent`，匹配规则是「行长 > 路径长度 + 阈值」或包含动词关键词；探针那一步会查 builtin path_rules 来提前禁掉无效菜单项（例如拖入 `~/.ssh` 时只显示 `[d][c]`）。
...
- 输入：`WorkspaceState { read_write, read_only, path_rules, agent_data_dir }`，由 `api/chat::compute_workspace_state(ctx)` 直接读 `ctx.gate.effective_roots()` / `ctx.gate.effective_path_rules()` 构造。
- 输出形如：
  ```
...
启动时 `chat_loop::print_startup_banner` 会把同一份 `WorkspaceState` 摘要打到 stderr，让人也能立刻看到当前权限边界，避免「为什么这次 read 被拒」的反复提问。

人 / Agent 两路输出共用同一份 `WorkspaceState`，确保「LLM 看到的边界」与「用户看到的边界」永远一致：
...
```text
                ...
                ┌────────────────┴──────────────────┐
                ▼                                   ▼
   build_system_prompt_with_state           print_startup_banner
   (system_prompt.rs)                       (chat_loop)
                │                                   │
                ▼                                   ▼
        WorkspaceStateSection                stderr 横幅
        priority = 150                       (人类可读，启动一次)
                │
                ▼
        SystemPromptBuilder
...
`gate` 是同一个 `Arc<dyn PermissionGate>`：原语执行、bash 检查、system prompt 渲染、startup banner 打印，全都对着这个对象的 `effective_*()`。会话期间的拖拽 / `[a]` / `[w]` 改动了 `SessionGrants` / `DraggedPaths`，下一次 system prompt 重建（compaction、子 agent 拆派等）会立即看到。
