...
| `agent_definition_dir` | `[agent].workspace` 或默认 `~/.pi_/workspace-main/` | 默认 read/write 授权根，`GrantType::AgentDefinitionDir` |
| `agent_trail_dir` | `~/.pi_/agents/<agentId>/` | sessions/logs/audit/tool-results 等运行态轨迹目录；按 readonly 暴露，敏感子路径由 builtin deny 保护 |
| `workspace.workspace_roots` | `~/.pi_/pi.config.toml [workspace] workspace_roots` | 全局用户工作区 read/write 授权根，所有 agent 共用 |
