...
**最终交付口径**：在 pi-rust-wasm 上对 **10–15 个** pi-mono **社区**插件完成**兼容测试**（与 Phase 0 扩展采样及 [`extension_compat_matrix.md`](../../../../docs/reports/extension_compat_matrix.md) 的规模对齐）。分层实施时，各 Tier 任务中的少量代表性扩展（如 Tier 2 的 2–3 个）用于**阶段性门禁与自动化回归**；**TASK-05e**（看板）在 TASK-05d 完成后按矩阵统一做端到端验收并勾选，汇总达到上述 10–15 个社区插件的总体兼容目标，避免仅验证内部或单一样本。
