...
本文档承接技术报告 [`docs/reports/multi-agent-openai-api-integration.md`](../../../docs/reports/multi-agent-openai-api-integration.md)：将其中 **与 pi-rust-wasm 架构冻结相关的结论** 收敛为 openspec 条目；**调研过程、五仓长篇对照与修订履历** 仍以报告为准。
...
| **pi-rust-wasm** | 完整 | `LlmProvider`、`OpenAiProvider` → Chat Completions |
...
```text
pi_agent_rust   hermes            openclaw         pi-mono              pi-rust-wasm（现状）
─────────────   ───────           ────────         ───────              ────────────────────
...
pi-rust-wasm **当下**已固定 **OpenAI 形 `ChatMessage`**，更接近 **wire 收敛**；若引入 **全局 IR**，属 **大改版**，需单独设计与迁移测试。
...
### 6.5 OpenAI **Responses** API（`POST /v1/responses`）接入——对标锚点与 pi-rust-wasm 落地点
...
#### 6.5.2 pi-rust-wasm 建议实施清单（与岔路 A 对齐）
...
下表是 **首次把 Responses 接入 pi-rust-wasm** 时的**项目级核对单**（含注册表与映射）；**基础设施稳定后**，新增同类 API 应 **收敛为：`XxxProvider` + 注册 id + 测试**。
...
| 2026-05-04 | **§6 重组**：冻结 **岔路 A**；新增 **§6.5** OpenAI Responses 接入——pi_agent_rust / openclaw / hermes **对标文件锚点** + pi-rust-wasm **实施清单**；§4 OGM 与 §2/§3 交叉引用更新 |
