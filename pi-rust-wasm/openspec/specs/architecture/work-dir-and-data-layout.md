...
- **全局额外授权目录**：除每个 agent 默认可写的 `agent_definition_dir`（`[agent].workspace` 指向的 `workspace-{id}/` 设计态目录）外，额外允许访问的外部目录根由主配置文件 **`pi.config.toml` 的 `[workspace] workspace_roots`** 列出（**所有 agent 共用**，不按 agent 分文件）。启动 `pi chat` 时的 shell 当前目录 `agent_workspace_dir` 不会自动进入授权根；需要通过 `pi workspace add --cwd`、cwd lazy prompt、`/path <路径>` 命令或本会话授权显式加入。由 `pi workspace add/list/remove` 或手编 TOML 维护。
