...
**作为用户**，我希望能快速安装、初始化pi-rust-awsm引擎，完成基础LLM配置，正常启动运行。
**验收标准**：
- [ ] 支持单文件二进制安装，无需额外依赖配置
- [ ] `pi-awsm init`命令可完成初始化配置，引导用户配置LLM API密钥、基础安全策略
- [ ] 配置文件采用AES-256加密存储敏感信息，无明文泄露
- [ ] `pi-awsm doctor`命令可检测运行环境、WasmEdge依赖、配置合法性，给出修复建议
...
- [ ] `pi-awsm plugin`系列命令可实现插件的加载、卸载、列表查看、启用/禁用
...
- [ ] API密钥加密存储，无明文泄露，调用支持限流与指数退避重试

### Story 8: CLI工具基础对话与会话管理
**作为用户**，我希望能通过CLI工具与Agent对话，管理会话历史，正常使用插件能力。
**验收标准**：
- [ ] `pi-awsm chat`命令启动对话模式，支持自然语言对话，流式响应渲染
...
- [ ] `pi-awsm session`系列命令可完整管理会话生命周期
