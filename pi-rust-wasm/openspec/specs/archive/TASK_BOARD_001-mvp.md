...
**目标**：消除 transcript 截断导致的 OpenAI 400；单次 API/Agent 错误不结束整段 `pi chat`；`pi.config.toml` 全局 `[workspace] workspace_roots` 与 `DefaultPrimitiveExecutor` 路径策略一致；`.env` 代理模板、确认框 `{agentId} · host|pluginId`、system prompt 防目录幻觉。
...
- [x] **P1** 从 `pi.config.toml` 加载全局 `workspace.workspace_roots` 并入允许路径；测试 + [`user-guide.md`](../docs/user-guide.md)（已废弃 per-agent `ext_workspaces.json`）
...
- [x] 12.9 workspace add/list/remove（持久化 `pi.config.toml` `[workspace] workspace_roots`；原 ext_workspaces.json 已废弃）
