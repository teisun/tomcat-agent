...
pub use core::{
    ...
    CheckpointError, CheckpointId, CheckpointKind, CheckpointMeta, CheckpointRecordRequest,
    CheckpointRestoreReport, CheckpointStore, Credential, DefaultLlmResolver,
    DefaultPrimitiveExecutor, DefaultToolRegistry, DenyAllConfirmation, DirEntry, EditFileResult,
    EditOperation, EditOperationType, ListOptions, LlmProvider, LlmResolver, LlmScene,
    ModelCatalog, ModelEntry, ModelThinkingStore, NoopStore, PrimitiveExecutor, PrimitiveOperation,
    ReadBinaryResult, ReadResult, ReadTextResult, ResolvedCall, RestoreOptions, ResumePlan,
    RetentionPolicy, SearchFileCount, SearchFileMatch, SearchFilesArgs, SearchFilesOutput,
    SearchFilesOutputMode, SearchFilesQuery, SearchFilesResultMode, SearchFilesStats,
    SearchFilesTarget, SessionTokenUsage, ShadowGitStore, StreamEvent, SwitchingCheckpointStore,
    ThinkingLevel, Tool, ToolCallInfo, ToolExecutor, ToolRegistry, UserConfirmationProvider,
    WriteFileResult, FILE_MAX_BYTES, IMAGE_MAX_BYTES,
};
pub use ext::{
...
pub use infra::{
    ...
    llm_http_status_error, load_config, load_config_for_init, load_config_toml_file,
    normalize_path, read_file_utf8, resolve_agent_definition_dir, resolve_agent_dir,
    ...
    EventListenerId, ExtensionEvent, FileAuditRecorder, HostcallAuditEntry, LlmConfig, LlmError,
    LlmErrorStage, LogConfig, PluginLifecycleAuditEntry, PreflightConfig, PrimitiveAuditEntry,
    PrimitiveConfig, ResumeHydrationMode, SecurityConfig, ServeConfig, ServeTransport,
    SessionConfig, ToolAuditEntry, TracingAuditRecorder, WorkspaceConfig, BRAND_ID, CLI_NAME,
