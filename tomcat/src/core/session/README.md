...
- **职责**：会话元数据 store（sessions.json）、对话 transcript（**pi-mono 相容** JSONL）的读写与 CRUD；CLI 子命令 init/doctor/config/session/plugin/audit，无参默认 chat（**CLI 入口与 chat 循环**见 [api/README.md](../../api/README.md)）。
...
- **路径**：由 `resolve_sessions_dir` 从 work_dir 推导，默认 `~/.tomcat/agents/main/sessions`；文件名为 `sessions.json`。
- **原子写**：`save_store` 使用 infra 的 `write_file_atomic`（写临时文件后 rename）。

### 2.2 对话 transcript（pi-mono 相容 JSONL）

- **首行**：SessionHeader（type、version、id、timestamp、cwd）。
- **后续行**：TranscriptEntry 枚举（message、model_change、thinking_level_change、session_info、**branch_summary**（上下文压缩摘要行，JSONL `type: branch_summary`）、label、custom）。与 **pi-mono 相容** JSONL 的其它 `type` 值并存时，未知行可能按 skip 策略处理；压缩语义以 `branch_summary` 为准。
...
- **上下文**：`init_context_state` 按天筛选 + 不足 10 向前补齐，构建 `ContextState`。`ContextState.messages` 是回合间的停车列表；运行中的 `AgentLoop` 通过 move 取得工作列表，并在退出时停回。
...
`tomcat chat`（或无参默认）进入交互式对话模式。核心文件：
- `src/api/chat/mod.rs` 等 — ChatContext、chat_loop、工具调用执行、CliConfirmation
- `src/api/render.rs` — MarkdownRenderer（流式代码块高亮，基于 syntect）

### 5.2 架构

`run_chat` → `ChatContext::from_config` → `tokio::Runtime::block_on(chat_loop)`。主循环：rustyline 读输入 → `init_context_state` 后从停车位 move 历史并追加当前输入 → AgentLoop 流式输出（MarkdownRenderer 高亮）→ 工具调用循环 → 消息经 append sink 写 transcript，列表在退出时停回。
