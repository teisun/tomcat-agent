...
//!    - `openai` → 挂在 `openai.rs`（文件内再分 `provider` / `stream` 子模块）；
//!    - `openai_responses_test` → 挂在 `openai_responses.rs`。

// `mocks` 需被 `tests/openai.rs` 通过 crate path 复用（该文件已由 openai.rs 内挂，
...
mod types_test;
// `openai` / `openai_responses_test` 由各自被测源文件通过 `#[cfg(test)] #[path]`
// 内挂为子模块，避免为测试放宽可见性
