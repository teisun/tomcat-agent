...
const BACKGROUND_BASH_NEXT_STEP: &str = "If your current step depends on this task, poll task_output(task_id, block=true, timeout_ms=...); otherwise keep working and a <background-task-finished> message will be injected automatically when it exits. Do not busy-poll.";
...
fn serialize_background_ticket(
...
mod tests {
    ...
    async fn background_ticket_keeps_fields_and_adds_next_hint() {
        ...
        let registry = Some(Arc::new(BashTaskRegistry::new(dir.path().join("tool-results"))));

        let text = handle_bash_background(
            &registry,
            SubagentType::User,
            "echo queued",
            None,
        ...
        assert!(value.get("startedAtUnixMs").and_then(|v| v.as_u64()).is_some());
        assert_eq!(
