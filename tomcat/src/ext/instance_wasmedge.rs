...
impl WasmInstance {
    ...
    fn run_script_file_impl(&mut self, script_path: &Path) -> Result<serde_json::Value, AppError> {
        let quickjs_path = self.quickjs_path.clone().ok_or_else(|| {
            AppError::QuickJS(
                "QuickJS wasm not found. Run `tomcat init` to extract embedded assets.".to_string(),
            )
        })?;
...
/// 优先级：`TOMCAT_QUICKJS_MODULES_PATH` → `~/.tomcat/assets/modules/` → `CARGO_MANIFEST_DIR/assets/modules`（开发兜底）。
fn resolve_quickjs_modules_dir() -> Option<PathBuf> {
    std::env::var("TOMCAT_QUICKJS_MODULES_PATH")
        .ok()
        .map(PathBuf::from)
        .filter(|p| p.is_dir())
        .or_else(|| {
