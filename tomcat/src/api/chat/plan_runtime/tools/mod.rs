...
//! | [`todos`] | 任何模式 | CHAT/Planning 写 session todo；EXEC 写 active plan | 我的待办 |
//! | `ask_question` | Planning | 透传 UI（P5 PR-AQ 落地） | 结构化提问 |
//!
...
pub enum ToolError {
    ...
    BadArgs(String),
    #[error("plan 文件错误: {0}")]
    PlanFile(#[from] super::file_store::PlanError),
    #[error("ops 错误: {0}")]
    Op(#[from] super::ops::OpError),
