...
/// - Chat → `""`
/// - Planning → `"[mode: PLAN]\n"`
/// - Executing { plan_id } → `"[mode: EXEC plan_id=<plan_id>]\n"`
/// - Pending { plan_id } → `""`（pending 期不进 LLM 转，由 `/plan build` 后切 EXEC 才贴）
/// - Completed { .. } → `""`（自动收口后回 CHAT 语义，提示已通过 reminder swap 移除）
pub fn user_prefix_for_mode(mode: &PlanMode) -> String {
    match mode {
        PlanMode::Planning => "[mode: PLAN]\n".to_string(),
        PlanMode::Executing { plan_id } => format!("[mode: EXEC plan_id={plan_id}]\n"),
