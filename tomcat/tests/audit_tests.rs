...
//! 黑盒测试，仅通过 tomcat 公共 API；使用临时目录隔离审计文件。
//!
...
fn test_audit_store_write_query_export_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let store = tomcat::AuditStore::open_if_enabled(&cfg)?
    ...
    recorder.record_primitive(PrimitiveAuditEntry {
        operation: tomcat::AuditPrimitiveOp::Read,
    ...
    let filter = tomcat::AuditFilter::default();
