...
2. **收集碎片**  
   ...
   - **feature 分支**：执行 `git branch --format='%(refname:short)' | grep -E '^feature/' | sort` 得到所有 **feature/** 分支列表（排序保证顺序稳定）。对每个分支：将分支名中的 `/` 替换为 `-` 得到 status 文件名（如 `feature/infra` → `feature-infra.md`）；若仓库根与当前项目根不同（如 monorepo），则路径带相对前缀（如 `tomcat/docs/status/feature-infra.md`）。对每个分支执行 `git show <分支>:<路径>` 读取该分支上的 status 文件内容；若某分支无该文件或读取失败，该节内容记为「（暂无进度碎片）」。
