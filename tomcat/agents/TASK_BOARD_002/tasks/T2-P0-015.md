...
- [ ] 文档：`docs/architecture/llm-multiprovider-integration.md` 新增 §6.5.4「Files 上传管理」节，写明 inline vs upload 决策树（< 1MB inline / 1–10MB upload+复用 / > 10MB 必须 upload）；`tomcat.config.toml.example` 注释 API key 需 `files` scope（默认开）；`core/llm/README.md` 补 upload 通道示例
