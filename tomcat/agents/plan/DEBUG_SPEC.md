...
| 目标 | 策略 |
| :--- | :--- |
| 测试 `pub(crate)` 类型（如 `CompactionResult`） | 放在**同级独立 `tests/` 子目录**中作为单元测试（父模块用 `#[cfg(test)] mod tests;` 引入；**禁止**在业务源文件内联 `#[cfg(test)] mod tests { ... }`，依据 [UNIT_TEST_LAYOUT_SPEC.md](../../openspec/specs/guides/testing/UNIT_TEST_LAYOUT_SPEC.md)） |
