...
> **⚠️ 过时提示（2026-04）**：本示例基于重构前架构，其中 `UserTurn`、`SummaryTurn`、`TurnEntry`、`user_turns_list`、`fold_entries_to_turns`、`on_new_user_turn`、`compound_turn_id` 等概念已在 `feature/collapse-to-chatmsg` 分支中删除。统一为 `ChatMessage` + `MessageKind`，详见 [docs/reports/collapse-to-chatmsg-guide.md](../../tomcat/docs/reports/collapse-to-chatmsg-guide.md)。本文仅保留作为计划格式范例参考。
