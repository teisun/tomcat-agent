...
| E2E-CLI-053 | 自动 | `test_user_switches_to_existing_session`                | 用户切换到已存在的会话               | `tomcat session new` → `tomcat session switch agent:main:main`  | exit 0                          |
| E2E-CLI-054 | 自动 | `test_user_switches_to_nonexistent_session_shows_error` | 用户切换到不存在会话时看到友好提示         | `tomcat session switch nonexistent-key`                        | exit 0；stdout 含"不存在"            |
