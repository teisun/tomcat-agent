...
*   **实时查看**：若需查看运行中的日志，使用 `--nocapture` 参数；**集成测试**建议：`RUST_LOG=tomcat=debug,info ./scripts/run-integration-tests.sh integration`，避免误以为未打日志。
*   **按顺序输出（便于阅读）**：默认多线程并行跑测试时，各用例的 Arrange/Act/Assert 日志会交错。若希望**同一测试的日志连续、按执行顺序**出现，可临时使用 `cargo test -j 1 ... -- --nocapture --test-threads=1` 调试；不要把调试命令当作默认门禁。
    ```bash
    RUST_LOG=tomcat=debug,info cargo test --lib -- --nocapture
    RUST_LOG=tomcat=debug,info ./scripts/run-integration-tests.sh integration
    ```
*   **级别控制**：利用 `RUST_LOG` 环境变量动态调整日志等级：
    ```bash
    RUST_LOG=tomcat=debug,info cargo test --test audit_tests -- --nocapture
    ```
