...
| P11 宿主函数注册面（本版新增）           | manifest 增 `functions[]` 静态契约；运行时增 `pi.registerFunction` / `__pi_functions` / `__pi_execute_function`；新增按 `point` 分发的宿主扩展点注册表与 `PluginFunctionInvoker`，并保证 host-facing function **不进入 `ToolRegistry`** | `src/ext/plugin/types.rs`(`functions[]`)、`assets/js/pi_bridge.js`(`registerFunction`)、`assets/js/pi_main_loop.js`(`__pi_execute_function` / `kind:"function"`)、`src/ext/plugin/function_registry.rs`（或等价 point registry 模块）、`src/ext/plugin_function_invoker.rs`（或泛化现有 `PluginToolExecutor`）、`src/api/chat/context.rs`(scope 绑定宿主扩展点注册表) | `plugin::tests::host_function_is_not_visible_to_llm_tool_registry`、`plugin::tests::registered_function_can_be_invoked_by_host`、`dispatcher::tests::function_call_roundtrip` | 给宿主用的能力单开一条函数面：静态可发现、运行时可执行、永远不污染 LLM 工具表。 |
