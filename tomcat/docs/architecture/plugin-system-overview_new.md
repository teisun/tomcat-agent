...
> **安装入口补充（T2-P1-017 PackageManager）**：plugin 的 runtime 发现链路仍是 `project(.tomcat/plugins) > agent(agents/<id>/plugins) > global(work_dir/plugins)`，安装/卸载/列包也已统一收口到 `PackageManager`：shell 走 `tomcat install` / `tomcat uninstall` / `tomcat packages`，会话内走 `/install`。安装成功后只会写三层 `plugins/` 正文目录与 per-layer `plugins/registry.json` / `packages/registry.json`；当前 code/claw 会话若通过 `/install` 安装，则只刷新 plugin catalog stub 与静态 `manifest.tools` / `manifest.functions` 可见集，**不会**在 install 路径调用 `load_plugin()`、不会启动 session VM，也不会热替换已加载实例。**host-facing `functions[]` / `FunctionRegistry` 与普通 plugin 复用同一套三层发现 / 安装链；差异不在发现根，而在 runtime 物化语义：按 `point` 做 `project > agent > managed` override，只把当前 scope 的赢家写入 `FunctionRegistry`。**
...
5. **插件 = 能力容器，不是工具本身**（本轮重点）：插件是「装好后会注册东西的程序」，但本版把它贡献的能力**显式分成两条注册面**：① 给 LLM 用的 **tools**，进共享 `ToolRegistry`，和内置工具同一条 tool-calling 通道；② 给宿主用的 **functions**，进共享的**宿主扩展点注册表**（下文有时沿用 `FunctionRegistry` 这个简称，但对 `web_search.backend` 这类点最终会物化成按 `point` 收敛后的赢家视图），由宿主按契约调用，**不进 LLM 工具列表**。`web_search` backend 插件走的是第二条路。它仍可挂事件钩子、注册命令。也就是说：**和 skill 那套「把目录塞进提示词、模型挑了再读正文」是不同通道；和 host-facing function 也不是一回事。** **注意：本版 host-facing function 的发现 / 安装根与普通 plugin 一致，复用 `project > agent > managed` 三层磁盘根；真正写入 `FunctionRegistry` 时再按 `point` 做 override，只保留当前 scope 的赢家视图。** → §4.3.3、§4.1 插件 vs 工具 / 宿主函数 / 能力注入 LLM / 加载路径行。
6. **作用域 = 分账，不混同**（本轮重点，含一处设计修正）：`ToolRegistry`/`plugins` 管理态与运行期 VM 仍然需要区分 session / scope / 进程级职责，但**不要把“project scope 共享”与“磁盘发现分层”混成一件事**。对 LLM 工具面，project scope 仍然是重要可见性单位；**对本版新增的 host-facing `functions[]` / `FunctionRegistry`，也复用同一套 `project > agent > managed` 发现链，但宿主消费面不是 candidate-union，而是按 `point` 做 override：高层一旦声明该 `point`，低层同 `point` provider 不再进入当前 scope 的有效视图。** 针对「插件很多别一开机全量加载」：开机只读 manifest 建轻量 Catalog，能力信息优先用 manifest **静态 `tools[]` / `functions[]`**（不跑码即可见），运行 `VmActor` **首次用到才建、闲了回收**。竞品里 openclaw 在“manifest 声明工具 + 按需子集加载”上最成熟，pi-mono/pi_agent_rust 对启用集仍 eager 全量（反面）。→ §4.3.5、§4.3.6、§4.1 注册面作用域 / 加载策略行。

> 一句话串起来：**换引擎（rquickjs）→ 同进程所以要软隔离 + 崩溃隔离 → 多会话并发所以要多实例 + sessionId 不串台 → 插件既可贡献给 LLM 的 tools，也可贡献给宿主的 functions，其中 `web_search` backend 走 function 面；宿主函数与普通 plugin 一样走三层发现 / 安装链，但 runtime 按 `point` 做 `project > agent > managed` override → 运行实例按会话懒加载，插件再多也只读名册、用到才加载。**



### A.0 Mermaid 时序图（插件注册：从磁盘到共享注册面）

> 专业：此图画的是**三阶段**，但**注意不是线性必经**：**阶段一 编目**（启动/发现期，只读 manifest、不跑插件代码；先形成 `PluginCatalog` 底座，等某个 scope 视图 materialize 时再把静态 `tools[]` 零跑码写入 `ToolRegistry`，把静态 `functions[]` 交给各自的**宿主扩展点注册表**）。**这里没有“宿主根单源例外”**：host-facing `functions[]` / `FunctionRegistry` 与普通 plugin 一样复用 `project > agent > managed` 三层发现 / 安装链；差异只在 materialize 时按 `point` 做 override，把当前 scope 的赢家视图写入注册表。**阶段二 激活/注册**是个**条件子步骤**，**仅对没有静态 `tools[]` 的 legacy 工具插件**才发生，触发点在**会话进入 / scope 首次激活**（用短命校验 VM 跑一遍命令式补登工具，使其对 LLM 可见——不能拖到 `tool_call`，否则 LLM 根本看不见）。host-facing `functions[]` 本版要求**静态在 manifest 声明**，但它表达的是**宿主可见的最小契约**，不是插件内部配置；例如 `web_search.backend` 只声明“这个插件提供 `webSearchBackend(params)` 入口”，插件内部有哪些后端、什么顺序、默认参数是什么，都留在 JS。**阶段三 运行**里再分两条：LLM 真发 `tool_call` 走工具面；宿主（如 `WebSearchRuntime`）按扩展点契约调用 function 走宿主面，二者都按会话多实例起/复用 VM 干活。它要回答四个问题：① `ToolRegistry`/宿主扩展点注册表/`plugins` 表的作用域（两者都复用 scope 视图，但函数面会按 `point` 收敛赢家）；② 插件很多时为什么不一上来全量加载；③ `PluginManifest`、Catalog、短命 `PluginVmInstance`、长命 `VmActor`、两类注册面、`PluginInstance` 分别何时出现、存什么；④ 为什么 host-facing function 不会污染 LLM 工具表。
> 说人话：LLM 工具面和宿主函数面别混着看。工具那边继续按项目视图组织；宿主函数这边也会扫 workspace / agent / global 三层，但同一个 `point` 只认更高层那一个赢家，低层同 `point` 不会混进来。只有当某插件**真的被用到**，才把它的代码拉起来跑；LLM 叫工具是一条路，宿主按扩展点契约调 function 是另一条路。注意：编目阶段**不跑代码**。
...
> - **① 程序启动（阶段一编目/预填，不跑码）**：先只读 manifest。对工具面，可继续沿 generic plugin catalog 预填共享底座；**对本版 host-facing `functions[]` / `FunctionRegistry`，也复用 `project > agent > managed` 三层发现 / 安装链，再按 `point` 把当前 scope 的赢家物化到宿主扩展点注册表。** 启动期**不跑任何插件代码**。注意 `PluginCatalog` 与内置工具的编译期 `BUILTIN_TOOL_CATALOG` 是两张不同的表，最终只有 `tools[]` 汇进某个具体 scope 的 `ToolRegistry` 给 LLM；`functions[]` 则按 `point` 分发到宿主扩展点注册表给宿主。
...
> - **宿主函数面**：发起者是 **宿主子系统**（例如 `WebSearchRuntime`），它按**扩展点契约**找到当前 scope 下该 `point` 的赢家函数；对 `web_search.backend` 这类点，宿主只认一个稳定入口 `webSearchBackend(params)`，并把显式 `backend` 或 `"auto"` 作为参数传进去；若当前赢家插件返回 `unsupported_backend`，宿主把它视为当前 scope 不兼容，**不会**再跨插件继续尝试，`auto` / vendor fallback 仅在该赢家插件内部完成。  
...
| **FunctionRegistry / 宿主扩展点注册表**            | 宿主函数的静态契约面 / 当前 scope 的有效赢家视图              | 与 `ToolRegistry` 分离；复用 `project > agent > managed` 三层发现与分层安装元数据；物化时按 `point` 收敛为当前 scope 的赢家集合（如 `web_search.backend`）                                   | 不污染 LLM 工具清单；高层同 `point` 覆盖低层；同层冲突 stable first-wins + warning                                 | 工具架给模型，函数表给宿主；这张函数表不是磁盘全量清单，而是“当前项目看见谁赢了”。           |
...
| G9 注册面分账共享 | `ToolRegistry`/`plugins` 与 `FunctionRegistry` 都按 project scope 组织；其中 `FunctionRegistry` 复用 `project > agent > managed` 发现链，但注册面按 `point` 做 override，只保留当前 scope 的赢家；运行实例才 per-session | 工具表可以按项目看；宿主函数也按项目看，但同一点位永远只留高层赢家。 |
...
| P11 宿主函数注册面（本版新增）           | manifest 增 `functions[]` 静态契约；运行时增 `pi.registerFunction` / `__pi_functions` / `__pi_execute_function`；新增按 `point` 分发的宿主扩展点注册表与 `PluginFunctionInvoker`，并保证 host-facing function **不进入 `ToolRegistry`** | `src/ext/plugin/types.rs`(`functions[]`)、`src/ext/plugin/catalog.rs`(`discover`)、`assets/js/pi_bridge.js`(`registerFunction` / `__pi_functions` / `__pi_execute_function_async`)、`assets/js/pi_main_loop.js`(`kind:"function"`)、`src/ext/plugin/function_registry.rs`、`src/ext/plugin_function_invoker.rs`、`src/api/chat/context.rs`(启动/refresh 时按三层 catalog 物化到共享 `FunctionRegistry`，并按 `point` override 收敛赢家) | `plugin::tests::function_registry_groups_functions_by_point`、`chat::tests::materialize_functions_into_function_registry_only`、`chat::tests::manifest_js_name_mismatch_surfaces_clear_error`、`quickjs_e2e_tests::e2e_real_function_plugin_roundtrip` | 给宿主用的能力单开一条函数面：静态可发现、运行时可执行、永远不污染 LLM 工具表；真正暴露前先按点位选赢家。 |
| P12 web_search backend 插件（Phase 1） | 官方插件 `web-search-backends`：manifest 静态声明 `functions[] = [{ point: "web_search.backend", function: "webSearchBackend" }]`；运行期 `pi.registerFunction("webSearchBackend", handler)`；插件内部自己维护支持的 backend 列表、默认配置与 auto/fallback 顺序；MiMo 首发；宿主 `web_search` 只通过 `backend`/`auto` 参数统一调入；`tomcat init` 默认把官方插件装进 global 层，但 project/agent 层可用同 `point` override 盖掉它 | `assets/plugins/web-search-backends/{plugin.json,main.js}`、`src/api/cli/init.rs`(`ensure_builtin_plugins`)、`src/core/tools/web_search/*`(把插件当成一条后端扩展槽位调用)、`src/core/llm/*`(MiMo 经 `createChatCompletion` 透传 `tools`/`annotations`) | `web_search_tool_tests::plugin_backend_explicit_mimo_roundtrip`、`web_search_tool_tests::plugin_backend_auto_slot_degrades_cleanly`、`cli_tests::init_installs_builtin_web_search_backends_plugin` | 先让 MiMo 以插件 function 方式接进来，把“宿主调插件后端”这条新路打通；后续如需项目级覆盖，也只改高层插件，不改宿主协议。 |
...
#### 4.3.5 注册面作用域：tool 面按 scope，宿主函数面按 point override

> 专业：这里必须把 **tool 面** 和 **host-facing function 面** 分开说。`ToolRegistry` / `plugins` 管理态仍可按 **per-project scope** 组织：一个 project = 一个 scope，同一 scope 内 sessions 共享工具可见集与插件管理态；运行实例 `VmActor` 继续按 `(session, plugin)` 隔离。**但本版新增的 `functions[]` / `FunctionRegistry` 既不再走“宿主根单源”例外，也不是把三层结果直接 candidate-union 给宿主调用方**；它复用 `project > agent > managed` 发现链，并在 `materialize_host_functions_from_catalog()` 阶段按 `point` 做 override：高层一旦声明某个 `point`，低层同 `point` provider 不再进入当前 scope 的有效视图。同层冲突按稳定扫描序 **first-wins + warning**。代码侧已有两个可复用锚点：① Code 模式已有按 `project_root(cwd)` 计算的 scope key（`src/core/session/scope.rs:49`）；② `SessionRuntimeRegistry` 已留 registry 形态占位（`session_runtime.rs:64`）。本期设计应把“项目 scope 共享”与“按 `point` 选赢家”显式区分，避免误把三层发现等同于三层 union。
> 说人话：项目 scope 这件事，工具和宿主函数都共享；区别只是工具面可以把当前项目能看见的工具都摆出来，而宿主函数面会先按 `point` 选赢家。你切到另一个 workspace，看到的可能是另一套函数赢家视图，但规则始终稳定：`project > agent > global`。

**为何 `functions[]` 要复用三层发现，但在注册面按 `point` override？**

> 专业：`tools[]` 与 `functions[]` 受众不同，但发现根不必分裂。`tools[]` 回答「这个项目给 LLM 什么能力」；`functions[]` 回答「这个项目 scope 下宿主子系统可以按哪些契约调插件能力」。两者都应复用同一套 `project > agent > managed` 发现 / 安装链，保持 PackageManager 与 runtime 一致；真正的差异放在注册面：`ToolRegistry` 保留完整可见集，`FunctionRegistry` 则把同一 `point` 的多层 provider 收敛成当前 scope 的赢家视图。
> 说人话：目录怎么找，工具和函数用同一套规则；真正特殊的是“同一个点位谁说了算”。这件事不要丢给调用方猜，而要在注册表里提前决出来。

归纳四条：

1. **受众分账**：LLM 工具面继续按 project scope 暴露；宿主 function 面仍服务 `WebSearchRuntime` 等子系统，但视图同样随当前 scope 解析。
2. **发现根对齐**：`functions[]` 的安装层、发现层、refresh 层全部复用 `project > agent > managed`，不再另造“宿主根单源”例外。
3. **调用面收敛**：同一 `point` 进入 `FunctionRegistry` 前先做 override；高层覆盖低层，同层冲突 first-wins + warning。
4. **回退边界清晰**：`unsupported_backend` 只允许在**当前赢家插件内部**做 auto/vendor fallback；不再跨插件继续尝试下一个 provider。

再把「声明」和「执行」分两层看：

| 层       | 是什么                     | 作用域                                                 | 载体                                          | 为什么这样                                       |
| ------- | ----------------------- | --------------------------------------------------- | ------------------------------------------- | ------------------------------------------- |
| 发现/编目   | 当前 scope 能找到哪些插件 | 三层根：global+agent 在 work_dir；project 随 `project_root` | `PluginCatalog`（待实现）/ scope 视图                      | tools 与 functions 共用同一套发现事实源                           |
| 注册面（声明） | 有哪些工具、哪些宿主函数真正对当前 scope 生效         | tool 面可 **per-project scope 共享**；function 面先按 `point` 选赢家    | `plugins` 表 + `ToolRegistry` + `FunctionRegistry` | 工具声明保留完整可见集；宿主函数声明先收敛成赢家视图 |
| 运行面（执行） | 插件正在哪个会话里活着             | **per-`(session,plugin)`** 隔离                       | `RuntimeManager` 的 `VmActor` 多实例            | 执行才有会话上下文、需并发隔离                             |

**需不需要改设计？答案是：要改，而且本期就做，但是“统一发现根 + 收紧注册面”这条路。**
...
- **实现上最小改动路径**：复用现有 `project_root`/`scope_key` 概念，把当前偏单实例的 `ChatContext/ScopeServices` 抬成“**scope 容器**”；`PluginManager` 不再只挂一份，而是挂到对应的 scope 容器上；`FunctionRegistry` 的刷新逻辑则在 scope 容器内按 `point` 物化赢家快照。

> **一句话**：作用域 = 「一个 project 一个 scope」；同 scope 会话共享，跨 scope 隔离；宿主函数不再搞宿主根单源例外，而是在当前 scope 内按点位选赢家。 
...
因为是**多 session 并发**架构，再补五件事：①**崩溃隔离**——每插件专属线程 + `catch_unwind` + 超时中断，崩了只标红关掉、不拖垮宿主（底层 C 级崩溃兜不住，已列风险）；②**多实例模型**——同插件被多会话用就发多份独立活体（`(session,plugin)` 一实例）；工具面与宿主函数面都按 project scope 组织，但 **host-facing function 进入 `FunctionRegistry` 前会按 `point` 做 `project > agent > global` override**；③**sessionId 流转**——事件按信箱隔离已对，会话读写要从全局 current 改成按会话号取（修现状缺陷）；④**两条注册面**——给 LLM 的 tools 进共享 `ToolRegistry`，给宿主的 functions 进共享宿主扩展点注册表，二者正交，host-facing function 不污染 LLM 工具表；⑤**插件即能力容器**——`web_search` backend 这类能力以后可以作为 `webSearchBackend(params)` 这类宿主函数挂进插件，而不必伪装成 LLM 工具，且插件内部自己管理 vendor 列表与 auto/fallback 顺序；**本版宿主函数与普通 plugin 共享三层加载路径，特殊规则只发生在按点位选赢家这一步。**