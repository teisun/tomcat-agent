...
| R5 DeepSeek continuity         | DeepSeek 历史里的 `reasoning_content` 怎么处理                | **采用 same-profile compatible replay：兼容 DeepSeek `reasoning_content` 默认回放；tool-call turn 继续记录 `had_tool_call` / `replay_requirement` 作为强约束 metadata。**                      | `tomcat/src/core/llm/openai.rs`；DeepSeek Thinking Mode 官方文档；`openclaw/src/agents/openai-completions-compat.ts`                                                  | 设计：把 `had_tool_call` 与 `replay_requirement` 保留在 transcript metadata，但不再把 `had_tool_call` 当成唯一 replay gate；理由：Thinking Mode 只说“无 tool call 时不必参与后续上下文”，并未要求必须 strip，且对标实现更接近统一回放。              | 未入选：统一 strip；拒因：tool-call turn 会 400。未入选：把 `had_tool_call` 作为唯一 replay gate；拒因：会把“可 replay”和“必须 replay”混成一个开关，并制造误导性 warning。                                 | DeepSeek 这边默认能带就带；tool turn 只是“更强要求”，不是唯一通行证。 |
...
| **P3** | DeepSeek `chat.completions` same-profile compatible replay（当前经 OpenAI-compatible adapter 接入）         | `src/core/llm/openai.rs`、`src/core/llm/replay_policy.rs`                                                                    | `deepseek_tool_turn_replays_reasoning_content`、`deepseek_non_tool_turn_roundtrip_replays_reasoning_content`          | 把 DeepSeek “兼容的都能续，tool turn 只是更强约束”落成代码。 |
...
- 先把边界说清楚：**本期不要求仓库先有独立 `provider=deepseek`**。当前规划是把 `src/core/llm/openai.rs` 视为 **OpenAI-compatible Chat Completions adapter**，通过 `provider="openai"` + `api_base="https://api.deepseek.com"` + `api_key_env="DEEPSEEK_API_KEY"` + `default_model="deepseek-v4-pro"` 去接 DeepSeek；`ThinkingFormat::Auto` 只按 `model` 选格式，`deepseek-chat` / `deepseek-reasoner` / `deepseek-v4-pro` / `deepseek-v4-flash` 都自动走 DeepSeek thinking wire。
...
- model split：`deepseek-reasoner` 与 `deepseek-chat` / `deepseek-v4-*` tool-sensitive thinking mode 必须是两类 profile，不能混用——前者**不能**回放历史 `reasoning_content`，后者按 tool-call 决定是否回放；其中 `deepseek-v4-pro` 与 `deepseek-v4-flash` 归一到同一 `deepseek-v4` family，允许同 family 切 model 后继续 replay。
...
#### 4.2.6 可 replay 窗口与出站收敛 / 告警聚合

前面 §4.2.1–§4.2.4 定义的是「账本里存什么、单条 turn 如何降级」。本节定义**出站时按窗口收敛**，解决两个实际问题：一次对话刷出大量 `reasoning continuity downgraded` warning，以及历史里夹带的旧 provider continuity 被逐条重复判定。

- **账本全量，出站精简**：transcript 仍是 continuity 的全量账本——hydrate（`chat_message_from_entry` 整条反序列化）与 `/model` 切换（`switch_current_model` 只改 `model_override` + 落 `model_change` 事件）都**不**清洗历史里的 `reasoning_continuation` / `continuity`。真正的精简只发生在 wire builder 出站克隆上，绝不回写主账本。
- **可 replay 窗口（`ReplayWindow`）**：出站时只让**最新 assistant turn** 与**当前 turn**（最后一条真实 user——`role=user` 且 `kind=Normal`，排除 steering / compaction summary——之后的消息）内的 continuity 参与 opaque/文本 replay；更早的历史轮次一律 `StripOpaque`：只保留可见正文 / tool-call，丢弃隐藏 blob，**不转文本**、**不告警**。
- **窗口内动作矩阵**（窗口外恒为 `StripOpaque`）：
...
| 字段                          | JSON 类型       | 必填  | 默认值                    | 适用场景                      | 说明                                                            | 说人话                |
| --------------------------- | ------------- | --- | ---------------------- | ------------------------- | ------------------------------------------------------------- | ------------------ |
| `profile_id`                | `string`      | 是   | 无                      | 所有 profile                | 例如 `openai.responses.default`、`deepseek.v4.reasoning_content`。 | 规则卡名字。             |
...
| 变量                                                                                                            | 取值           | 含义                                                       | 优先级               | 说人话                                                  |
| ------------------------------------------------------------------------------------------------------------- | ------------ | -------------------------------------------------------- | ----------------- | ---------------------------------------------------- |
| `TOMCAT__LLM__REASONING_CONTINUITY__ENABLED` / `[llm.reasoning_continuity] enabled`                           | `true/false` | 是否开启 continuity capture + replay 主能力；默认 `true`。 | env > config > 默认 | 续传能力默认就开着；只有想静默回退旧行为时才显式关。                                         |
...
> 告警收敛：上面 A / B 两类「真正降级失败」由 `ReplayDowngradeReport` **每请求聚合成至多一条** warning（见 §4.2.6），而非逐条 turn 刷屏。


| 风险点                                            | 归一化处理                                       | 说人话             |
| ---------------------------------------------- | ------------------------------------------- | --------------- |
| DeepSeek tool-call turn 预期 replay 但历史缺 `reasoning_content` | 不做本地硬拦截；warning 后按当前 transcript 继续发送，由 provider 决定是否返回 400。若该轮本来没开思考模式，则视为普通 tool turn，不构造 continuity。 | 能降级就降级；没开 thinking 就别硬补。 |
...
| 单元      | `replay_policy_deepseek_v4_non_tool_turn_keeps_reasoning_content`     | ✅ 2026-06-02 | DeepSeek 普通多轮 continuity snapshot 保留且同 profile 可 replay。                      |
...
| 集成      | `deepseek_chat_roundtrip_replays_tool_turn_reasoning_content`       | ✅ 2026-06-01 | DeepSeek（经 `provider=openai` + DeepSeek `api_base` + `default_model=deepseek-v4-pro`）真接口已验证 tool-call turn 的 capture -> replay 闭环；thinking format 自动按 model 选 DeepSeek。 |
| 集成      | `deepseek_switch_model_roundtrip_replays_tool_turn_reasoning_content` | ✅ 2026-06-01 | 同一 DeepSeek family（`deepseek-v4-pro -> deepseek-v4-flash`）里，通过函数切 session `model_override` 后继续 replay，能续聊且不会触发 continuity 协议错误。 |
...
| 观察指标    | G1–G6 映射到上面 8 条用例 + docs review                                     | ✅ 2026-06-01 | 核心锁点与真接口验收已闭环。              |
...
1. **继续修正旧规则**：`DeepSeek = resend 时 strip thinking` 与 `had_tool_call=false 就绝不 replay` 都过粗；应改为“兼容 continuity 默认可 replay，tool turn 只表达更强约束”。
...
| 2026-06-02 | 新增 §4.2.6「可 replay 窗口与出站收敛 / 告警聚合」：出站按 `ReplayWindow` 只 replay 当前 turn + 最新 assistant，旧轮次静默 strip；逐消息 warn 收敛为 `ReplayDowngradeReport` 每请求一条汇总告警（A 同 profile 不兼容 / B 窗口内 continuity 彻底丢失）；同步更新 §10 错误模型。 |
