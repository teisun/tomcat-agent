...
└──────────────────────────────────────────────────────────────────────────────┘
  ...
  └──────────┬───────────┘   - DeepSeek / chat.completions / v4-pro
             |
             v
  ┌──────────────────────────────────────────────────────────────────────────┐
  │ Provider 请求 / 流式响应                                                 │
...
| `previous_response_id`       | OpenAI Responses 的会话优化锚点，可让下一轮引用上一轮 response。                                                            | `reasoning_continuation.provider_refs.openai_response_id`（建议形状）。                                         | **只能**作为 OpenAI 专属优化，不能作为跨 provider continuity 的主设计。                                                    | OpenAI 自家的高速路入口，不是通用账本。                      |
...
| R4 OpenAI Responses continuity | OpenAI 路径靠什么续推理                                       | **采用显式 replay reasoning item（含 `encrypted_content`）；`previous_response_id` 仅作可选优化。**             | `tomcat/src/core/llm/openai_responses/payload.rs`；`pi_agent_rust/src/providers/openai_responses.rs`；`hermes-agent/agent/transports/codex.py`                    | 设计：显式 replay 才是通用 continuity 主链，`previous_response_id` 放到优化层；理由：这样即使 `store=false` 也不丢跨 turn reasoning。   | 未入选：只保留 `previous_response_id`；拒因：不满足 shared transcript、也无法覆盖 DeepSeek。                                                      | OpenAI 这边先保证“自己带历史也能续上”，再谈快车道。        |
...
| **P3** | DeepSeek `chat.completions` tool-call-sensitive replay                                               | `src/core/llm/openai.rs`、`src/core/llm/replay_policy.rs`                                                                    | `deepseek_tool_turn_replays_reasoning_content`          | 把 DeepSeek “该带的带、该删的删”落成代码。 |
...
- capture：从 `response.reasoning_*` 事件收集 `thinking_text`；从完整 reasoning item 里收集 `encrypted_content` 或等价 opaque payload。
- persist：把 `response_id` 作为 provider reference 存起来，但不让它取代 transcript。
- replay：同 profile 下显式把 reasoning item 回放进 `input`；若开启优化，再附带 `previous_response_id`。
- fallback：若 replay blob 无效，允许 **降级为仅可见历史**，并把 `previous_response_id` 当作可丢弃优化重试一次。
...
- model split：`deepseek-reasoner` 与 `deepseek-v4-* thinking mode` 必须是两个 profile，不能混用。
...
```jsonc
{
  ...
  "store": false,
  "previous_response_id": "resp_abc123" // 可选优化，不是主账本
}
```
...
│   · 组 request body；保持 store=false                                       │
│   · previous_response_id 仅为优化分支                                       │
├──────────────────────────────────────────────────────────────────────────────┤
│ src/core/llm/openai.rs                                                     │
│   · chat.completions 出站组装                                               │
│   · DeepSeek reasoning_content capture/replay                               │
...
│   NewTurn    │─────────────▶│ VisibleOnly  │
└──────┬───────┘              └──────┬───────┘
       │ capture blob                │ profile mismatch
       ▼                             ▼
┌──────────────┐ compatible   ┌──────────────┐
│ OpaqueReady  │─────────────▶│ KeepOpaque   │
└──────┬───────┘              └──────────────┘
       │ mismatch + fallback_text
       ▼
┌──────────────┐
│ Downgraded   │
└──────┬───────┘
...
| `TOMCAT__LLM__OPENAI_RESPONSES__USE_PREVIOUS_RESPONSE_ID` / `[llm.openai_responses] use_previous_response_id` | `true/false` | 是否启用 OpenAI `previous_response_id` 优化；默认 `false`。        | env > config > 默认 | OpenAI 快车道单独开。                                       |
...
OpenAI previous_response_id 失效 → warning + 去掉该优化重试一次
...
| OpenAI `previous_response_id` 过期 / 不匹配         | 视为优化失效，去掉该字段后重试一次。                          | 快车道堵了，就退回普通车道。  |
...
| 维度      | 用例 / 编号                                                             | 状态           | 说人话                                                |
| ------- | ------------------------------------------------------------------- | ------------ | -------------------------------------------------- |
| 单元      | `replay_policy_openai_responses_keeps_encrypted_reasoning`          | PENDING      | OpenAI 同 profile 时要保留 opaque reasoning。            |
| 单元      | `replay_policy_deepseek_tool_turn_requires_reasoning_content`       | PENDING      | DeepSeek 调过工具的那轮必须回放 reasoning。                    |
| 单元      | `replay_policy_deepseek_non_tool_turn_strips_reasoning_content`     | PENDING      | DeepSeek 普通多轮不能乱带旧 reasoning。                      |
| 单元      | `cross_provider_downgrade_prefers_fallback_text`                    | PENDING      | 跨 provider 时优先退到安全文本。                              |
| 单元      | `transcript_roundtrip_preserves_reasoning_continuation`             | PENDING      | JSONL 来回一趟不能把 continuity 字段丢掉。                     |
| 集成      | `openai_responses_roundtrip_replays_reasoning_items`                | PENDING      | OpenAI Responses 真正能 capture -> persist -> replay。 |
| 集成      | `deepseek_chat_roundtrip_replays_tool_turn_reasoning_content`       | PENDING      | DeepSeek tool-call continuity 真闭环。                 |
| 集成      | `legacy_transcript_without_continuity_fields_still_hydrates`        | PENDING      | 老 transcript 继续能聊。                                 |
| 单元 / 集成 | `streaming_think_scrubber_hides_split_tags`                         | PENDING      | `<think>` 被拆成多段时也不能漏到用户。                           |
| 观察指标    | G1–G6 映射到上面 8 条用例 + docs review                                     | PENDING      | §3 里承诺的行为要有对应锁点。                                   |
...
2. **明确 OpenAI 定位**：`previous_response_id` 是优化，不是 shared transcript 的替代品。
