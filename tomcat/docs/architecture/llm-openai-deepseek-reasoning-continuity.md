...
| 集成      | `deepseek_chat_roundtrip_replays_tool_turn_reasoning_content`       | ✅ 2026-06-01 | DeepSeek（经 `provider=openai` + DeepSeek `api_base` + `thinking.format=deepseek`）真接口已验证 tool-call turn 的 capture -> replay 闭环。 |
