...
4. **`_start` 常驻但生命周期可控**
   ...
   - 空闲：`waitForEvent` → `rx.recv()` 挂起线程，不 busy-loop。
   - 微任务 drain：每轮事件处理后通过 `setTimeout(loop, 0)` 让出控制权，由 wasmedge-quickjs 已有的 `run_loop_without_io()` 自动 drain Promise 微任务和 tick tasks，再进入下一轮 `waitForEvent`。无需额外 Rust 代码。
...
event_n
  ...
  -> handler run (may create Promises)
  -> setTimeout(loop, 0), loop() returns
  -> run_loop_without_io() drains Promises + tick tasks
  -> setTimeout callback fires -> re-enter loop()
  -> waitForEvent blocks again

session_end / shutdown
  -> send {type: '__shutdown'} event
  -> loop() returns without setTimeout
  -> run_tick_task() == 0 -> run_loop_without_io exits
  -> _start returns -> actor stopped + resources cleaned
```
...
// 使用 setTimeout 而非直接递归或 while(true)，
// 让 QuickJS run_loop_without_io() 在两次 waitForEvent 之间
// 自动 drain Promise 微任务和 tick tasks。
(function sessionEventLoop() {
    function loop() {
        ...
        if (!res.ok || (res.data && res.data.type === '__shutdown')) {
            return;  // 不 setTimeout，run_tick_task()==0，整条链路干净退出
        }
        ...
        // 让出控制权：QuickJS event loop drain Promises，再进入下一轮
        setTimeout(loop, 0);
    }
    loop();
})();
...
**两层事件循环协作**：`loop()` 首次调用 `waitForEvent` 阻塞在 Rust `rx.recv()` 上；事件到达后 handler 执行、`setTimeout(loop, 0)` 返回 → `_start` 返回 → wasmedge-quickjs 的 `run_loop_without_io()` 接管：先 `JS_ExecutePendingJob` drain Promise 微任务，再 `run_tick_task()` 触发 `setTimeout` 回调重新进入 `loop()` → 下一轮 `waitForEvent` 阻塞。这利用了 `wasmedge-quickjs/src/event_loop/mod.rs` 已有的 `EventLoop` 和 `quickjs_sys/mod.rs` 的 `run_loop_without_io()`，无需额外 Rust 代码。
...
**当前短生命周期不是 wasmedge-sdk 的限制，而是 tomcat 自己选择的**。`instance_wasmedge.rs` 里每次 `run_script_file_impl` 都新建 `Vm`，执行完就丢弃。
...
- [ ] 事件队列上限、超时、清理与恢复策略
- [ ] `__pi_start_event_loop` 采用 `setTimeout(loop, 0)` 与 QuickJS `run_loop_without_io()` 集成（不使用直接递归或 while(true)）
...
- [ ] handler 内创建的 Promise 在下次 `waitForEvent` 阻塞前被 resolve（`run_loop_without_io` drain 生效）
