...
| 属性     | pi-mono (TS)                               | pi-agent-rust (Rust+QuickJS)      | tomcat 当前状态                     |
| ------ | ------------------------------------------ | --------------------------------- | ------------------------------------- |
...
| 分类       | 具体方法                                                                 | pi-mono                 | pi-agent-rust          | tomcat host-call-protocol |
| -------- | -------------------------------------------------------------------- | ----------------------- | ---------------------- | ------------------------------- |
...
宿主只暴露一个 `host.call(name, input_json)` 入口——和 tomcat 的 `__pi_host_call` 思路一致（单入口多路复用），通过 JSON 里的 `capability` / `method` 路由。
...
## 4. tomcat 定制 wasm 模块现状

当前 tomcat 的 `host-call-protocol.md` 和 `host-api-layer.md` 已规划了 7 大类 module：
...
tomcat 和 pi-agent-rust 有一个关键区别：

- **pi-agent-rust**：直接在 Rust 中嵌入 QuickJS（rquickjs crate），可以向 QuickJS 注入任意多个原生函数
- **tomcat**：Rust → WasmEdge → `wasmedge_quickjs.wasm` → QuickJS，中间隔了一层 wasm，只能通过 wasm import 注入函数
...
桥接脚本作为一个独立的 `.js` 文件放在 tomcat 仓库里（如 `assets/js/pi_bridge.js`），不编译进 wasm。执行流程变为：
