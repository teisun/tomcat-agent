...
- **tools**：`registerTool`、`unregisterTool`、`getToolList`、`callTool`、`getActiveTools`、`setActiveTools`、`registerCommand` 等，params 见 dispatcher 与 pi-mono 约定。  
- **events**：`on`（别名 `subscribe`）、`off`、`once`、`emit`，params 为事件名与 payload。  
- **session**：`getCurrentSession`、`getMessages`、`sendMessage` 等，params 见实现。

### 4.5 agent

- **log** / **debug** / **info** / **warn** / **error**：`params.message` (string)。
- **sendMessage**：`params.message` (any)，向 agent 会话发送消息。
- **sendUserMessage**：`params.content` (any)，以用户身份注入消息。

### 4.6 context（事件 ctx 代理）

由 `pi_bridge.js` 中 `__pi_dispatch_event` 构建的 ctx 对象的动态方法调用，详见 [JS 桥接层](js-bridge-layer.md) §4.2。

- **isIdle**：无 params，返回 `{ idle: boolean }`。
- **abort**：无 params，请求中止当前操作。
- **getCwd**：无 params，返回 `{ cwd: string }`。
- **getModel**：无 params，返回 `{ model: object | null }`。
- **hasPendingMessages**：无 params，返回 `{ pending: boolean }`。
- **shutdown**：无 params，请求关闭。
- **getSystemPrompt**：无 params，返回 `{ prompt: string }`。
- **getContextUsage**：无 params，返回 `{ tokens, contextWindow, percent }`。
- **compact**：`params.options` (object, 可选)，触发上下文压缩。
- **uiNotify**：`params.message` (string), `params.type` ("info"|"warning"|"error")。
- **uiSelect**：`params.title` (string), `params.options` (string[])，返回 stub。
- **uiConfirm**：`params.title` (string), `params.message` (string)，返回 stub。
- **uiInput**：`params.title` (string), `params.placeholder` (string)，返回 stub。
...
**导航**：返回 [插件系统全貌](../plugin-system-overview.md) | 上一节：[宿主API层](host-api-layer.md) | 下一节：[JS 桥接层](js-bridge-layer.md)
