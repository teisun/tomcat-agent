...
本文为 [Architecture](../../Architecture.md) 插件系统的补充设计，承接 [JS API 与 pi-mono 对齐](js-api-alignment.md) 和 [Phase 2 长生命周期 VM](phase2-long-lived-vm.md)，聚焦 **pi-mono 社区扩展在 tomcat 上零修改运行** 的技术架构决策。
...
**tomcat 的目标**：实现与 pi-mono 一致的用户体验——用户直接写 `.ts`，放到约定目录，零构建自动加载运行。

### 社区插件兼容测试总规模

**最终交付口径**：在 tomcat 上对 **10–15 个** pi-mono **社区**插件完成**兼容测试**（与 Phase 0 扩展采样及 [`extension_compat_matrix.md`](../../../../docs/reports/extension_compat_matrix.md) 的规模对齐）。分层实施时，各 Tier 任务中的少量代表性扩展（如 Tier 2 的 2–3 个）用于**阶段性门禁与自动化回归**；**TASK-05e**（看板）在 TASK-05d 完成后按矩阵统一做端到端验收并勾选，汇总达到上述 10–15 个社区插件的总体兼容目标，避免仅验证内部或单一样本。
...
pi-mono 扩展是 TypeScript 源码，依赖 npm 包（`@mariozechner/pi-coding-agent` 等）。tomcat 运行在 WasmEdge QuickJS 上，QuickJS 不能直接执行 TS，也不能解析 npm 包。
...
**tomcat 可参考移植的部分**：SWC 编译流程、CJS→ESM 转换、Node.js 虚拟模块的 JS polyfill、API 签名设计。

**tomcat 与 pi_agent_rust 的关键差异**：tomcat 使用 WasmEdge 运行 QuickJS（`.wasm` 二进制），pi_agent_rust 使用 rquickjs（原生 QuickJS Rust 绑定）。模块加载器的集成方式不同——WasmEdge 通过文件系统 preopen 提供模块，rquickjs 通过 Rust trait 实现自定义 loader。
...
| `readFile` / `writeFile` / `editFile` | 有 | 无（pi-mono 不暴露） | tomcat 独有 | — |
...
- **tomcat 当前**：脚本顶层执行，`globalThis.pi` 预注入
...
**TASK-05a 起已启用**：`assets/modules/` 由 wasmedge-quickjs 同步拷贝，`instance_wasmedge.rs` 在 WASI 中增加 `./modules` → 该目录的 preopen；可选环境变量 `TOMCAT_QUICKJS_MODULES_PATH` 覆盖路径。以下「缺失模块」表仍描述相对完整 Node 的差距。
...
| TASK-05e | 10-15 社区插件矩阵端到端验收（看板 T1-P1-002e） | 2-4 | 环境/版本差异与验收记录维护 |
| **合计** | Tier 1-2（实现） | **10-15** | — |
| **合计** | 含 Tier 3-4（实现） | **18-27** | — |

Tier 1-2 为核心交付目标，Tier 3-4 为 P2 可延后。**TASK-05e** 为矩阵级验收，**依赖 TASK-05d**，工作量单独一行（不计入上两列「合计」的 Tier 实现人天）。
