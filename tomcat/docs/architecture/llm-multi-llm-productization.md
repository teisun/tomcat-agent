...
> - OpenAI 直连与兼容网关并存的推荐范式是：保留 `gpt-5.4`，另加 `gpt-5.4_gateway`，并用 `model_name = "gpt-5.4"` 把本地 id 与上游真名解耦。
>
...
| **Model Catalog** | 模型清单：id → 元数据（api、provider、base_url、能力、窗口） | 计划新增 `core/llm/catalog.rs` + 内置表 + 用户 `models.toml` 覆盖 | 单一事实源；UI/Resolver/运行时都读它 |
...
```text
  models.toml(用户)         内置表(catalog.rs)
        ...
        └──── merge(覆盖) ───────┘
                 ...
                 ▼
        ModelEntry { id, api, provider, base_url?, capabilities, context_window? }
...
| 集成 | `tests/llm_tests.rs::{catalog_resolver_e2e,scene_compaction_uses_default_or_selected_model}`（mock provider） | PENDING | 拼起来跑一遍。 |
