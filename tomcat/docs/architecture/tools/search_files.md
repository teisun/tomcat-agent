...
- [7. 启动预检（tomcat chat）](#7-启动预检tomcat-chat)
- [8. 预检事件状态机](#8-预检事件状态机)
...
├──────────────────────┼──────────────────────────────────────────────────┤
│ tomcat search_files  │ 单工具双 target（content / files）+ 同上 + 增加：  │
...
| 实施点 | 交付范围（含交付物） | 主要代码落点（含落地点） | 验收锚点（示例） |
|--------|----------|--------------|------------------|
| **单次合入** | Tier1（`rg`+`fd`）+ Tier2（`ignore` + globset + regex）自动回落；`tomcat chat` 启动预检与安装探测；`warnings` / `implementation` 审计字段 | 见 [§4 One-Glance Map](#4-one-glance-map文件职责总览) | 见 [§12 测试矩阵](#12-测试矩阵实现--用例)（全表函数名 + 状态列） |
...
│    Windows 由宿主在 output 后汇总写入                        │
│  • tracing target：tomcat_preflight（RUST_LOG=debug）       │
...
- ~~只在 `tomcat init` 检查 / 安装 rg/fd~~ → 否：与 `search_files` 触达点错位；改为 `tomcat chat` 入口预检。
