...
| 维度 | **tomcat**（现状） | **pi-mono** | **pi_agent_rust** | **openclaw** | **hermes-agent** | **cc-fork-01**（plan 锚点） | 说人话 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **E1** | 链式替换；schema 单段 | `edits[]` + legacy 单段合一；**对原文** | 单段 `edit` + 独立 `hashline_edit` 多段 | **`pi-tools` `edit`** + 条件 `apply_patch` | `patch` 与 file 工具同族 | 单段对外 + 内部多段 patch 工具 | 多段编辑到底对「哪一版全文」动手，各家做法差很多。 |
