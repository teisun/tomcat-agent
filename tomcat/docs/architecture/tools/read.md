...
| 来源               | 键                                               | 含义                | 备注                                                                                           |
| ---------------- | ----------------------------------------------- | ----------------- | -------------------------------------------------------------------------------------------- |
| `tomcat.config.toml` | `[tools.read] max_bytes`                        | 文本路径**无窗口**时的字节上限 | 默认 25 MiB；[`infra/config/types.rs`](../../../../src/infra/config/types.rs) `ToolsReadConfig` |
...
| `mtime` 欺骗式不变     | 陈旧漏判        | 存 `content_hash` + hashline 给 edit 侧纵深；[§8](#8-状态机与会话表) 写明边界         |
