...
|------|-----------|-----------|------|--------|--------|
| **`todos`** | `~/.tomcat/agents/<agentId>/todos/<session_id>.todo.md`（session 路径） | **CHAT / PLAN / EXEC / Pending / Completed**；EXEC 期仍可调 | 个人 / 会话级待办；**不**写 plan.md，和 `update_plan` 的 PlanFile 待办并存 | `apply_todos_op(TodoStore)` | 聊天里随手记的清单。 |
