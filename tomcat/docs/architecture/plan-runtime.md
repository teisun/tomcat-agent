...
- 提示词规则：`AgentMode::Plan` 注入 planner reminder；`executing_plan_id().is_some()` 注入 executor reminder。两者位于 request-local ephemeral tail，不写入 system prompt、transcript 或 `ContextState.messages`。
...
### Prompt-cache tool surface（2026-08）

内置工具目录是会话的稳定缓存前缀：除了没有任何可见 skills 时不下发 `load_skill`，所有 mode 都收到同一份 tools 数组。`create_plan`、`ask_question` 和写路径限制由各自 handler 与 `tool_exec` 的路径 policy 执行，并返回带下一步指引的 `RejectedInMode`，而不是从目录中消失。

```text
single ToolSurface observation
       ├── function definitions sent to provider
       └── system prompt tool_lines

mode / grants / active plan
       └── ephemeral request tail only
```

该决定应当被推翻的唯一条件是：某个工具即使被 handler 拒绝也会产生不可接受的安全、计费或可用性副作用，且不能在请求发出前用稳定的 schema 或无状态 validation 消除。届时应先为该工具拆分安全的只读 declaration；只有确认不存在这种拆分路径，才允许重新引入按 mode 过滤目录，并必须同时测量缓存命中损失。
...
**说人话**：这篇文档回答四件事。第一，**PLAN 模式不是 LLM 工具**——`/plan` 是本地 slash 命令切换会话状态；写盘边界仍由 `safety` 强制执行。第二，`/plan build <plan_id/path>` 是唯一进入执行态的入口；reviewer 仅辅助，不做 verdict gate。第三，**`todos` 和 [`update_plan`](./tools/update-plan.md) 任何模式都可见**——`todos` 写 `TodoFile`（session 本地 scratchpad，固定到 `agents/<id>/todos/<session_id>.todo.md`），`update_plan` 写 `PlanFile.frontmatter.todos[]` / `milestones[]`（按 `plan_id` 或显式 `path` 路由；EXEC/Pending 缺省跟随当前 active plan path）；二者共享 `apply_todos_op` op 引擎。**`TodosRuntime` / `PlanRuntime` 都是 per-session OOD 对象，挂在 `ChatContext` 上**。第四，reviewer 是 internal subagent dispatch（与 codex `codex_delegate.rs` 同构），由 `CreatePlan` 工具内部同步阻塞调用，**不**进 catalog。

> **当前实现覆盖说明（2026-08）**：为了稳定提示缓存，LLM 的 built-in tool
> catalog 不再随 CHAT / PLAN / EXEC mode 增删，所有内置工具始终发给模型；
> mode 与路径限制仍由各 tool handler 和 `safety` 强制执行。PLANNER /
> EXECUTOR reminder 与当前权限状态以请求末尾的 ephemeral
> `<system_reminder>` 发送，不写入 system prompt、`ContextState` 或 transcript。
> 下方早期 PR 记录中“catalog swap / 注入 system 区段”的表述仅描述历史实现，
> 与这条当前契约冲突时一律以本框为准。
...
| **TodosRuntime** | 单 session 内 `TodoFile` 的 IO 入口 | `ChatContext.todos_runtime: TodosRuntime`；详见 §6 | per-session 单实例；构造时绑定 `base_dir + session_id`，负责 `.todo.md` 原子写 | 一个聊天会话一个 todo 落盘管家。 |
...
| **G9 Runtime per-session OOD** | `TodosRuntime` 与 `PlanRuntime` 都是 per-session 单实例，挂在 `ChatContext` 上；多 session 由未来 `ChatContextRegistry` 处理 | 每个会话各自一套大管家，不共享全局 HashMap。 |
...
| **R10 Runtime OOD** | `TodosRuntime` / `PlanRuntime` per-session，挂 `ChatContext`；多 session 由未来 `ChatContextRegistry` 解决 | 全局 HashMap 会让 mode 切换、面板刷新失去会话隔离 | 一会话一套大管家。 |
...
|--------|----------------------|--------------------------|------------------|--------|
| **PR-PLA 命令面与 PLAN 模式骨架** | `/plan` / `/plan exit` / `/plan build <plan_id/path>` 本地命令；`AgentMode` / `PlanRuntime` per-session 对象；请求末尾注入对应 `<system_reminder>`、CLI prompt 统一走 helper；help 文案；详见 [`tools/planner.md`](./tools/planner.md) | `src/api/chat/commands/{parse.rs,cmd_help.rs,cmd_plan.rs}`、`src/core/plan_runtime/`、`src/api/chat/prompt.rs`、`src/api/chat/mod.rs` | 见 §11：`parse_plan_commands`、`prompt_helper_renders_plan_modes` | 先把命令族、PLAN 模式与 prompt 渲染搭起来。 |
| **PR-PLB `todos` / `update_plan` / `CreatePlan` / `AskQuestion` 工具与计划文件** | built-in `todos`（任何模式可见，写 TodoFile）；[`update_plan`](./tools/update-plan.md)（任何模式可见，写 PlanFile.frontmatter todos/milestones）；`CreatePlan`（详见 [`tools/create-plan.md`](./tools/create-plan.md)）；`AskQuestion`（详见 [`tools/ask-question.md`](./tools/ask-question.md)）；`TodosRuntime` / `PlanRuntime` 内部状态机 | `src/core/tools/contract/catalog.rs`、`src/core/agent_loop/tool_exec.rs`、`src/core/plan_runtime/{file_store.rs,todo_runtime.rs,mod.rs}`、`src/core/tools/plan_tool/{create_plan.rs,update_plan.rs,ask_question.rs,todos.rs}` | 见 §11：`todos_returns_full_items_snapshot`、`update_plan_visible_in_all_modes`、`plan_file_round_trip_frontmatter`、`create_plan_only_visible_in_planning_mode`（PENDING） | 四个工具落地，状态机串通。 |
...
- **交付**：`/plan`、`/plan exit`、`/plan build <plan_id/path>` 全部在 `parse.rs` 内被识别为本地命令并转入 `cmd_plan.rs`；`PlanRuntime` per-session 对象先落地；每次请求按 PLAN 或 executing 生命周期将对应 `<system_reminder>` 追加到 ephemeral tail；user/agent prompt 统一由 `src/api/chat/prompt.rs` 渲染；`/help` 文案补齐。
...
- **稳定 catalog**：`tool_exec` 每轮下发同一份内置工具集；`create_plan`、`ask_question` 与 plan 更新操作的可执行性由 handler 根据 `current_mode()`、计划生命周期和调用参数判定。PLAN 期写盘路径仅 `~/.tomcat/plans/*.plan.md`，由 `safety.rs` 路径策略统一拦截；执行中计划文件仅允许通过 `update_plan` 变更。详见 [`tools/planner.md`](./tools/planner.md) §6。
...
┌──────────────────┐                          │ ephemeral tail 加 PLANNER/EXECUTOR reminder
│ cmd_plan.rs      │ 调 PlanRuntime API（enter/exit/build）
└──────┬───────────┘
       │  enter Planning / Executing
       ▼
   swap request-tail reminder + retain stable catalog + (build only) inject user meta plan body
       │
       ▼
...
**说人话**：先把命令族、PLAN 模式、请求尾部 reminder 与稳定工具目录搭好；普通输入永远不会被偷偷当成 `/plan`。
...
- **`todos` 可调时机（硬约束）**：
  - `mode == Chat` → 写 `TodoFile`（active `~/.tomcat/agents/<agentId>/todos/<session_id>.todo.md`）；
...
- **单写入路径（todos 工具）**：
  ```
  validate(todos) → acquire_file_lock(active_file) → mutate(TodosRuntime/PlanRuntime)
...
- **subagent 隔离**：reviewer 起一个 `AgentLoop` 子任务，注入专属 `SubagentContext { system_prompt, allowed_tools, transcript=fresh, parent_session_id, token_budget }`；`allowed_tools` 由 internal dispatch 路径**硬编码**为 `{read, grep, find, todos}`（默认）或 `{read, grep, find, todos, update_plan, edit}`（当 runtime 配 `allow_review_edit=true`）；**永不**含 `create_plan`（防套娃）；**禁止**调 `bash` / `write` / `dispatch_agent` / `checkpoint`。子 Agent 的完整对话会 best-effort 落盘到 `agents/<agentId>/subagent-sessions/{child_session_id}.jsonl`，格式与主 session transcript 同构，但不进入历史会话列表。
- **reviewer 输出契约**：reviewer 最终消息含 `summary:` 自由文本（建议简明聚焦，但 runtime 不再做固定字符上限截断），可选用 `update_plan` 修订 frontmatter `todos[]`、或用 `edit` 改 plan 正文；**无 verdict 字段**；runtime 把摘要与修改说明写入 `transcript.plan.review` 事件，**不**回写 `.plan.md` 中的审稿块，也**不**改 `mode`。
...
- 永远写 `TodoFile`（`~/.tomcat/agents/<agentId>/todos/<session_id>.todo.md`），**不写** `PlanFile`；
...
## 6. TodosRuntime / PlanRuntime per-session OOD 设计

### 6.1 设计原则

| 原则 | 内容 | 说人话 |
|------|------|--------|
| **per-session 单实例** | `ChatContext` 持有 `todos_runtime: TodosRuntime` 与 `plan_runtime: PlanRuntime`，每个 session 各自一份 | 一个会话一套大管家。 |
| **不维护全局 HashMap** | 多 session 由未来 `ChatContextRegistry` 处理；`TodosRuntime` / `PlanRuntime` 自身**不**承担“按 session_key 路由”职责 | 不在 runtime 里塞会话表。 |
| **内存映射 + 文件同步** | `PlanRuntime` 持有 session-local todos 的内存态；`TodosRuntime` 只负责把这份状态同步到 `TodoFile`；`PlanFile` 仍由 `PlanRuntime` 路径管理 | 内存和磁盘各管一层，但通道唯一。 |
| **panel 是只读投影** | 内存状态变化时通过 channel 推 panel；panel 不回写 | 屏幕只播不录。 |

### 6.2 `TodosRuntime`（CHAT 路径）

```rust
pub struct TodosRuntime {
    base_dir:    PathBuf,   // ~/.tomcat/agents/<agentId>/
    session_id:  String,    // 当前真实 session_id
}

impl TodosRuntime {
    pub fn new(base_dir: PathBuf, session_id: String) -> Self { /* 装配时捕获 */ }
    pub fn persist(&self, file: &TodoFile) -> Result<PathBuf> {
        // 1. path = base_dir/todos/<session_id>.todo.md
        // 2. create_dir_all(path.parent())
        // 3. write tmp with frontmatter(session_id) + body
        // 4. rename tmp -> final
        // 5. return final path
    }
}
...
**说人话**：`TodosRuntime` 现在只是当前 session 的「TodoFile 落盘管家」——构造时一次性绑定 `base_dir + session_id`，之后每次 `todos` 工具成功调用，就把 `PlanRuntime` 里的 in-memory scratchpad 覆盖写到同一个 `todos/<session_id>.todo.md`。它不再 hydrate、多文件列举、也不再碰 `sessions.json.activeTodosId`。
...
impl PlanRuntime {
    ...
    pub async fn apply_todos_op(&self, ops: Vec<TodosOp>, replace: bool) -> Result<TodoResult> {
        // 与 TodosRuntime 路径类似，但这里写的是 PlanFile.todos[] + frontmatter
...
| 关系 | 说明 | 说人话 |
|------|------|--------|
| `ChatContext ⊃ TodosRuntime` | 每个会话独占一份 `TodosRuntime` | 每个会话一份 todo 落盘大管家。 |
| `ChatContext ⊃ PlanRuntime` | 每个会话独占一份 `PlanRuntime` | 每个会话一份 plan 大管家。 |
| `TodosRuntime / PlanRuntime ↔ panel_tx` | 内存改完通过 channel 推 panel | 屏幕只读。 |
...
>
> - `ChatContextRegistry`：key = `session_key`（持久 chat 身份），value = `Arc<ChatContext>`，存 `TodosRuntime` / `PlanRuntime` / 共享 `Arc` 服务 / `root_session_id`，与 chat session 同寿。
...
```text
  ...
  EXEC mode
     ...
     |                                                                            |
    +-------------- all work todos terminal (via update_plan in EXEC) ----+
    |                                                                  |
    |                                                                  v
    |                                                NextAction:
...
**说人话**：EXEC 期推进 plan 走 `update_plan`（默认指向 `active_plan_id`）；它复用 `todos` 的 op 引擎写 `PlanFile.frontmatter.todos[]` / `milestones[]`。work todo 做完后由 `NextAction` 指向可见的 review gate、修复、验收或交还；验收靠 verify skill 和后台任务证据，而不是旧 verifier。`todos` 仍可用作个人 scratchpad，写自己的 `.todo.md`，不动 plan。

#### 7.2.1 `verify_gate` 语义（休眠资产，非当前完成路径）

`Verifier`、`PlanRuntime::dispatch_verifier` 和 `[plan].verify_gate` 保留以支持历史 transcript 回放和未来实验，但当前 `update_plan` **不读取也不派发**它们，不能用此配置开启、关闭或解释当前收口。当前完成路径见 [plan-exec-code-verification.md](plan-exec-code-verification.md)：review 结果由 `NextAction` 分流，最后通过受管 `verify` skill 和 `BashTaskRegistry` 证据完成 Acceptance。
...
| 多 session 串扰 | 中 | `TodosRuntime` / `PlanRuntime` 都是 per-session 单实例；多会话由 `ChatContextRegistry` 路由 | 一会话一份大管家。 |
...
| ~~`TodosRuntime` / `PlanRuntime` 用全局 `HashMap<session_key, _>`~~ | **替代**：per-session 单实例，挂 `ChatContext`；多会话由未来 `ChatContextRegistry` 路由。 | 一会话一份大管家。 |
...
- [`session-storage.md`](./session-storage.md)：transcript 自定义事件与历史扩展字段说明（含已废弃的 `activeTodosId` / `todos_ids`）。
...
**一句话总结**：Tomcat 的 `PlanRuntime` 采用「**本地 `/plan` slash 切 PLAN/EXEC 模式与 catalog**、**`CreatePlan` LLM 工具创建 PlanFile 并内部派 reviewer**（仅辅助）、**`/plan build <plan_id/path>` 是 EXEC 唯一入口**、**built-in `todos` 改状态并返回完整 snapshot**、**计划文件做 durable source**、**cancel_token 转 pending 可续跑**、**`TodosRuntime` / `PlanRuntime` per-session OOD 挂 ChatContext**、**CLI prompt 统一显示当前模式**」的组合路线。本文是运行时主 spec，工具粒度细节散落在 [`tools/`](./tools/) 子目录。
