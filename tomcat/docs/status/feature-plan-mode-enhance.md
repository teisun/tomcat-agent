...
- [/] **P9 verifier（PR-V0～V3）** `DOING` Owner=Tom — 方案 [`plan-exec-code-verification.md`](../architecture/plan-exec-code-verification.md) 已按拍板裁剪定稿（**frontmatter 零改造**；唯一配置 `[plan].verify_gate: "soft"|"gate"`，默认 `"soft"`；不加 `/plan verify` CLI；不分 `verify_task_type`；`VerifySummary` 双通道回传 = transcript `plan.verify` + `update_plan` tool result.verify）。当前阶段＝规格定稿 + 起草 [`PLAN_verifier.md`](../../agents/plan/PLAN_verifier.md)；实施按 PR-V0 → V1 → V2 → V3 串行（PR-V1+ 依赖本分支已就绪的 reviewer 派发基础设施 ✅）。子项与验收锚点见 [`T2-P1-002.md`](../agents/TASK_BOARD_002/tasks/T2-P1-002.md) **PR-V0～V3** 与方案 [§4.2](../architecture/plan-exec-code-verification.md#42-实施点分期)。
