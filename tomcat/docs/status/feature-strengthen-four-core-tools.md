...
- **新配置**：`[tools.bash].timeout_ms` / `[tools.bash].max_output_chars` / 环境变量 `TOMCAT__TOOLS__BASH__*`，默认 120_000ms / 30_000 chars。
...
- **新配置**：`[tools.write] normalize_crlf` / 环境变量 `TOMCAT__TOOLS__WRITE__NORMALIZE_CRLF`，默认 `true`。
...
- [✓] **[P0]** 验证：`cargo test -p tomcat` 全量（含集成与 wasmedge e2e）通过。
