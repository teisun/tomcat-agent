...
| :--- | :--- | :--- | :--- | :--- |
| Nibbles | 2026-07-11 17:33 +0800 | ACTIVE | feature/add-model-form-redesign | — |
...
### 🔌 INTERFACE (接口变更)
- Webview↔Host：新增 `searchContext` 请求/响应与 context search 相关消息；Composer 经 Suggestion 插件驱动，关闭统一 `exitSuggestion` / `closeMention()`。
...
### 集成说明
- 本分支目标：Add Model 双模式 Tab + 官方 seed 语义；叠加 Composer `@` 提及上下文搜索（零后端，仅扩展侧）。
- `@` 行为：有工作区时敲 `@` 出下拉；无工作区/切会话经 `closeMention()` 关闭；选中注入 context chip，不改 serve/wire。
- 验收：扩展 `gate:fast` + `test:integration` + Devhost E2E（文件/目录/无工作区）；`package:vsix` / 纯插件包不入库。
