...
- [✓] **[P0]** 分支侧门禁：`cargo fmt --all`、`cargo clippy --all-targets -- -D warnings`、`cargo test --lib`、`cargo test --test cli_tests -- --test-threads=1`、`RUST_LOG=tomcat=debug,info ./scripts/run-integration-tests.sh all`（第二次复跑全绿）。
