...
**结论：** 只有通过了这些“折磨”的集成测试，我们的 `tomcat` 引擎才真正具备了生产级发布的资格。
...
- 门禁仍以全量分类执行 `./scripts/run-integration-tests.sh integration` 通过为准；鲁棒性覆盖由「各 `*_tests.rs` 内边界用例」+「`robustness_tests.rs` 中跨模块/通用用例」共同满足。
