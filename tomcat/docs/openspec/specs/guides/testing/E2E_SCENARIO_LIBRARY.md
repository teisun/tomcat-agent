...
| E2E-VSCEXT-033 | 自动 | `transcript_rich_rendering_opens_code_card_titles_and_inline_paths` | 用户在 TranscriptView 里看到富渲染 assistant 消息：markdown 标题正常、代码/ASCII 围栏被裱成卡片、正文反引号路径变可点击，点击代码卡片标题或正文路径都能在编辑器打开并定位到指定行 | 创建 webview session → 注入一条含 `##` 标题、` ```lang path:line ` 围栏、` ```text ` ASCII 围栏与内联 `` `path:line` `` 的 assistant 消息 → 抓 DOM snapshot → 分别点击代码卡片标题与内联路径 | DOM snapshot 中 `assistantCodeCardCount >= 2`、`assistantClickablePathCount >= 1`；HTML 含标题/路径/ASCII 内容；点击代码卡片标题后，宿主活动编辑器打开对应文件并选中围栏里的 `line`；点击内联路径后，同文件重新定位到 inline `path:line` 指定行。当前自动化由 `gui/src/components/markdown/ChatMarkdown.test.tsx`、`src/ide/tests/diff_apply_edit.test.ts`、`src/ui/webview/tests/provider.test.ts`、`src/ui/webview/tests/state.test.ts` 与 host E2E `assertTranscriptRichRenderingFlow` 共同锁定 |
