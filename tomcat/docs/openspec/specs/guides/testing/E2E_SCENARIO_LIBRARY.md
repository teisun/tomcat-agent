...
| E2E-CLI-016D | 自动（需 `DEEPSEEK_API_KEY`） | `test_user_background_bash_blocking_waitslice_real_llm_cli` | 用户要求助手在后台 bash 后立即进入 `task_output(block=true)`；这次等待不再被中途日志打断，而是睡到 `timeout`，再从返回的 tail 里看到新进展后收尾 | `tomcat code` + 单轮 prompt：后台 bash `sleep 2; echo TOKEN_WAITSLICE; printf BLOCKWAIT_DONE > file; sleep 30`，必须用 `task_output(block=true, timeout_ms=5000)` + `task_stop` 完成，不准 `task_output(block=false)` | exit 0；stdout 含 `BLOCKWAIT_OK`；非 TTY stderr 不含 `waiting_for_output`；transcript 至少 1 次 `task_output(block=true, timeout_ms=5000)`，并出现 `wakeReason=timeout` + `TOKEN_WAITSLICE`，随后出现 `task_stop`；产物文件内容正确 |
| E2E-CLI-016E | 自动（需 `DEEPSEEK_API_KEY`） | `test_user_background_bash_multiple_timeout_slices_real_llm_cli` | 用户要求助手在同一后台任务上经历至少两次 timeout slice，再继续等到某次 timeout 的 tail 中出现目标 token | `tomcat code` + 单轮 prompt：后台 bash `sleep 12; echo TOKEN_MULTI_TIMEOUT; printf MULTI_TIMEOUT_DONE > file; sleep 30`，必须重复 `task_output(block=true, timeout_ms=5000)` 直到看到 token，再 `task_stop` | exit 0；stdout 含 `MULTI_TIMEOUT_OK`；非 TTY stderr 不含 `waiting_for_output`；transcript 中 `task_output` 至少 3 次、timeout 结果至少 2 次，最终出现 `wakeReason=timeout` + `TOKEN_MULTI_TIMEOUT` 与 `task_stop`；产物文件内容正确 |
...
| E2E-CLI-102 | 自动 | `serve_prompt_failed_turn_retry_keeps_previous_response_id_hint` | 同一 session slot 在瞬时失败（网络波动 / 403）后直接重发，**改动A 已回退**：不再清 `previous_response_id`，重发保留上一轮续写句柄（问题3 根因待 `TOMCAT-DIAG` 日志判定，不再锚定 `previous_response_id`） | 预置一条带 `resp_turn1` continuity 的 assistant 历史 → `prompt` 命中 mock 失败 → 不切 session 直接再次 `prompt` | 第一次失败请求体带 `previous_response_id=resp_turn1`；recover 后第二次请求体**仍带** `previous_response_id=resp_turn1`；重发成功收敛到 `agent_end`。当前自动化由 `api::serve::tests::commands_test::serve_prompt_failed_turn_retry_keeps_previous_response_id_hint` 锁定 |
...
| E2E-VSCEXT-005 | 自动 | `test_webview_single_frontend_multisession_switch_restore` | 用户在单前端 webview 下创建并切换多个 session，确认切走/切回后 transcript、busy 状态、历史分页与 draft 都不串台 | 打开 webview → 新建 session A / B → 在 A 发送 prompt 并制造 live / history 状态 → 切到 B 再切回 A | `activeSessionId` 切换正确；A/B timeline 与历史游标各自独立；切回 A 后原 busy/live transcript 仍在；**不再存在 ownership / read-only conflict banner / owner 接管语义**。当前自动化由 host E2E `assertWebviewMultiSessionFlow` 与 `assertWebviewSessionSwitchRestoreFlow` 锁定 |
...
## Story 8e — VSCode webview transcript 仿 Chat 体验
...
| E2E-VSCEXT-010A | 自动 | `test_user_webview_collapsed_group_activity_ticker_rolls_and_hides_cleanly` | 用户不展开 thinking 组，也能看到当前组内工具进度 | 触发 live cluster：同一折叠 `ThinkingGroup` 中连续注入多个 context tool，覆盖“工具完成 → LLM 静默间隙 → 下一工具到来 → turn 结束”全链路 | DOM：仅活跃折叠组出现 `group-activity-ticker`；ticker 与展开子行共用子项缩进/连接线，静止时也一眼能看出是分组子项；live context 工具执行期间 `group-activity-ticker` 下方始终有恒高 `progress-row` 作为底部锚（ticker 不是 live-cluster 最后一个子元素），连续滚动时底边不再随 followBottom 闪/抖；快工具不被跳过、静默间隙最后一项 linger + shimmer；turn 结束时先滚到空行再平滑收拢消失；无思考文本回合从 1→2 个 context 工具不发生整段 remount |
...
| E2E-VSCEXT-016A | 自动 | `test_user_webview_background_bash_loading_and_task_output_countdown` | 用户让 bash 长任务转后台后，既要看见 bash 卡仍在跑，也要看见**唯一一个**一等公民倒计时行；连续多轮轮询不刷屏不抖动，静默间隙末尾仍有 `...` 呼吸点 | 打开 webview → 注入 `bash(run_in_background=true)` complete 回执（含 `backgroundTaskId/backgroundRunning=true`）→ 注入 `task_output(block=true)` running/complete 多个 slice（含 `task_output(block=false)`、`task_stop`、`task_list` 交错）→ 最后注入 `background_task_finished` 或等价终态 snapshot | DOM：bash 卡标题先显示 `Running in background` 且 shimmer、生效时 `statusVariant=running`；**任一时刻只有当前 running 的 `task_output(block=true)` 一行**不进 thinking 组、以独立 action row 原地逐秒显示 `Waiting up to 9m59s…`，slice 结束翻成 `Waited for shell` 并**折进分组**（不再堆积成一排独立行）；同序列的 `task_output(block=false)`（`Read output <id>`）、`task_stop`（`Stopped <id>`）、`task_list`（`Listed tasks`）始终留在折叠组内、图标可区分；连续多个 slice / bash 卡片刷新时 timeline 引用稳定（`React.memo`+reconcile 生效）——两帧 DOM 对未变卡片保持同引用、无重排闪烁、视口不被反复拽底；LLM 静默间隙对话流末尾出现 `ProgressRow` 的 `...`（即使存在 todos）；后台完成后 bash 卡翻成 `Ran`（非零退出码显示 `Ran · exit N`）且 shimmer 消失 |
...
| E2E-VSCEXT-019A | 自动 | `test_user_webview_plan_event_rows_and_single_create_card` | transcript 中的 plan 证据必须拆成“唯一 create 卡 + 多条 update 增量行”，连续勾选不能再被折叠吞掉或重复造卡 | webview 新建 session → 注入 `create_plan` running/complete + `plan.create` / `plan.todos` → 连续注入两次 `update_plan` complete（各勾 1 个 todo）→ 抓 DOM snapshot | DOM：`planCardCount===1`；`toolTitles` 同时包含 `Checked 1 · 1/3` 与 `Checked 1 · 2/3`；不得残留 `Creating plan` 行；`groupFoldTitles` 不得回声 `Creating/Updated/Checked plan`；`View Plan` / `Build` 只出现在 create 卡，不得在每条 update 行重复 |
...
| E2E-VSCEXT-030 | 自动 | `structured_diff_is_single_source_of_truth_in_webview` | 用户点击 `View diff` 时，webview 必须从 Rust `tool.diff` 还原完整 before/after；即使宿主编辑器较窄，也不能被 VSCode 自动折叠成“看起来像左栏空了”的单页 | 触发已有文件编辑回合 → 保持默认窄宿主布局 → webview 点击 `View diff`（走 `openReconstructedDiff`） | 宿主 `preparedChanges.originalContent` 等于完整改前文本、`proposedContent` 等于完整改后文本；webview `provider` 优先走 `openReconstructedDiff`；若用户未显式配置 `diffEditor.renderSideBySideInlineBreakpoint`，扩展会把有效 breakpoint 设为 `0`；左栏非空、diff 保持双栏。当前自动化由 `src/ide/tests/diff_apply_edit.test.ts`、`src/ui/webview/tests/provider.test.ts` 与 host E2E `assertWebviewDiffFlow` 共同锁定（`@tomcat` participant 已删除，原 `render_message_stream.test.ts` 随之移除） |
| E2E-VSCEXT-031 | 自动 | `failed_turn_history_renders_error_summary_and_expandable_detail` | 用户在失败轮里既要看到 live 阶段的错误，也要在回合收敛后被 transcript 中的结构化摘要替换，并可展开/复制脱敏后的原始错误 | 触发 `agent_end{error}` 先渲染 live raw error → 下一次 `agent_idle` 触发 history hydration → 点击 `Show original error` / `Copy original` | 普通 superseded 消息仍隐藏；失败 user 保留可见；live raw error 会在 `agent_idle` 后替换成 `summary`+`detail`；error 气泡默认只显示 `summary`，展开后显示 `detail`，复制走脱敏后的原文；`detail` 中 `Cookie/Set-Cookie/x-api-key/password/secret/Basic/sk-*` 必须被 `[REDACTED]`，同时 `Host` / `Request-Id` 仍可见。当前自动化由 `src/ui/webview/tests/provider.test.ts`、`src/ui/webview/tests/state.test.ts` 与 `gui/src/components/MessageBubble.test.tsx` 锁定 |
| E2E-VSCEXT-032 | 自动 | `settings_key_fields_visible_boxes_stay_aligned_after_webview_ready` | 用户打开 Add Model 弹窗时，`Key slot` 与 `API key` 两个可见控件外框必须顶边对齐、同高，且 DOM 快照只能在 settings webview 真正 ready 后采样 | 打开 settings → 进入 Add Model 弹窗 → 等待 `settings.ready` → 抓取 `settings-key-slot-box` 与 `settings-api-key-input` 的可见矩形 | `keySlotBox.top` 与 `apiKeyInput.top` 差值 ≤ 1px；两者 `height` 差值 ≤ 1px；若未收到 `settings.ready`，不得提前消费 DOM 快照。当前自动化由 host E2E `assertSettingsKeyFieldsAligned`、`gui/src/settings/SettingsApp.test.tsx` 与 `src/ui/settings/SettingsPanel.test.ts` 共同锁定 |
| E2E-VSCEXT-033 | 自动 | `transcript_rich_rendering_opens_code_card_titles_and_inline_paths` | 用户在 TranscriptView 里看到富渲染 assistant 消息：markdown 标题正常、代码/ASCII 围栏被裱成卡片、正文反引号路径变可点击；**流式进行中已完成的代码块应立即带 `hljs-*` 颜色，而不是等整条消息 settle 后再补色**；点击代码卡片标题或正文路径都能在编辑器打开并定位到指定行 | 创建 webview session → 按 token/delta 分段注入一条含 `##` 标题、` ```lang path:line ` 围栏、` ```text ` ASCII 围栏、`` `path:line` `` 与 ` ```mermaid ` 的 assistant 消息 → 在流式尚未结束时抓 DOM snapshot → 收尾后再抓一次 → 分别点击代码卡片标题与内联路径 | 流式尚未结束时，先完成的代码块已经带 `hljs-*` token span（证明 `highlight.js` 同步上色 + 按块 memo）；最终 DOM snapshot 中 `assistantCodeCardCount >= 2`、`assistantClickablePathCount >= 1`，且 mermaid 收尾后才出现 `<svg>`；HTML 含标题/路径/ASCII 内容；点击代码卡片标题后，宿主活动编辑器打开对应文件并选中围栏里的 `line`；点击内联路径后，同文件重新定位到 inline `path:line` 指定行。当前自动化由 `gui/src/components/markdown/ChatMarkdown.test.tsx`、`gui/src/components/markdown/markdownRuntime.test.ts`、`gui/src/components/markdown/richRenderRuntime.test.ts`、`src/ide/tests/diff_apply_edit.test.ts`、`src/ui/webview/tests/provider.test.ts`、`src/ui/webview/tests/state.test.ts` 与 host E2E `assertTranscriptRichRenderingFlow` 共同锁定 |
| E2E-VSCEXT-034 | 自动 | `long_streaming_turn_uses_incremental_session_frames_without_full_repaint` | 用户在超长流式回答中持续看到丝滑输出，而不是每个 token 都触发整份 transcript 重发 | 打开 webview → 构造 500 条 timeline 基线 → 对同一 assistant turn 连续注入大量 `message_update` delta → 在 turn 边界再注入 `turn_end` | 宿主热路径优先发 `sessionPatch`（必要时 `sessionView` 兜底），而不是为每个 delta 重发整份 `state`；GUI 侧仅尾部条目换引用，其余条目引用复用，未命中热路径的整份重绘次数不随 delta 线性增长。当前自动化由 `src/ui/webview/tests/stateBroadcaster.test.ts`、`src/ui/webview/tests/provider_broadcast.test.ts`、`gui/src/stateReconcile.test.ts`、`gui/src/statePatch.test.ts` 与 `gui/src/App.sessionFrames.test.tsx` 共同锁定，手工基线/after 用 `gui/scripts/measure-webview-bridge.tsx` 复验。 |
| E2E-VSCEXT-035 | 自动 | `background_session_streaming_does_not_drag_foreground_timeline` | 用户正在看 session B 时，session A 在后台继续流式输出，不应该反复把前台 transcript 整段重寄或打断当前视图 | 打开 webview → 新建 session A/B → 切到 B → 仅向 A 注入 `message_update` / `tool_execution_*` / `turn_end` → 再切回 A | 前台 activeSession 维持 B，不出现跳转；后台 A 的热更新只进入自己的 `sessionPatch` / `sessionView` 桶，不重寄 B 的 timeline；切回 A 后能看到完整最新 transcript。当前自动化由 host E2E `assertWebviewMultiSessionFlow`、`src/ui/webview/tests/provider_broadcast.test.ts` 与 `gui/src/App.sessionFrames.test.tsx` 锁定。 |
...
| E2E-PROMPT-029 | 自动 | `standards_6_7_8_are_byte_identical_in_core_identity_planner_and_reviewers`（`src/core/prompts/tests/load_test.rs`） | 工程规范 #6/#7/#8 在常驻身份、PLAN 提示词与两类 reviewer 提示词中一字不差，防单边漂移 | `core_identity.txt` + `planner.txt` + `reviewer/*.txt` | S6（第一性原理/极致优雅/敢推翻坏设计）、S7（人话+总览及每复杂章节 ASCII 图 + 创建/更新研发计划时完整写入计划本身）、S8（UI 从 UX 出发并优先遵循用户项目现有风格）三句逐字同时出现在四个模板 |
