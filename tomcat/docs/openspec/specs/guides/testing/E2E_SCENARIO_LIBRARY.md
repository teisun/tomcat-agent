...
| E2E-CLI-067 | 自动 | `test_user_chat_skill_list_reload_use` | 用户在 chat 内列出 / 重载 / 显式使用 skill，且 user-only skill 可被点名 | `tomcat chat` 或 inprocess chat loop：先 `/skill list`，再修改 `.tomcat/skills/` 后执行 `/skill reload`，最后 `/skill use secret "summarize request"` | `/skill list` 输出含 source / user-only 标签；`/skill reload` 后下一轮 prompt 使用新的 `SkillSet`；`/skill use` 向当前轮注入 `<skill>` 正文与 intent，且不因 `disable-model-invocation` 被拒 |
