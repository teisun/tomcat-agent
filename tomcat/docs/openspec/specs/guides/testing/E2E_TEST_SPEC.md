...
本文档定义 tomcat 项目的端到端（E2E）测试类别、场景来源、编写标准与执行规范。完整用户操作模拟场景清单见 [E2E_SCENARIO_LIBRARY.md](E2E_SCENARIO_LIBRARY.md)。
...
所有 E2E 测试命令**必须**包含 `RUST_LOG=tomcat=debug,info`，确保 INFO/DEBUG 日志全部输出：
