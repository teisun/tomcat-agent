...
run_session 单测依赖 TOMCAT__STORAGE__SESSIONS_DIR，并行时多用例写同一路径导致偶发失败。改为每用例 tempdir 并 canonicalize 后 set_var，保证进程内路径唯一，消除竞态。
...
```
chore(doc): 更新 docs/status/feature-wasm-plugin.md 宪法流程走查项
