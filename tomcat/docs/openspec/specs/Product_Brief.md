# tomcat - 安全自进化AI Agent运行时
## 项目定位
tomcat 是一款基于 Rust 构建的轻量、高安全、可自进化的 AI Agent 核心运行时。宿主侧提供会话、多模型 LLM、Agent 循环与内置工具；插件侧通过进程内 `rquickjs` 运行 JS/TS 插件，在保持扩展性的同时统一收口敏感能力与审计。

## 问题陈述
1.  部分 Agent 运行时对插件或工具开放无限制系统权限，插件可直接访问宿主，缺少沙箱隔离，存在安全隐患
2.  现有 AI Agent 插件体系要么能力边界僵化，要么开放裸系统权限，难以兼顾灵活性、可扩展性与安全性
3.  常见方案将扩展能力与 JS/TS 强绑定，缺少 Rust 实现的高性能、高可靠宿主运行时，复杂任务自举能力受限
4.  多 Agent 协作与 Skill 工作流若与核心引擎强耦合，会导致引擎臃肿，新手上手门槛高
5.  部分 Wasm 插件方案开发成本高、与既有 JS 插件契约兼容性差，社区资产难以平滑迁移
6.  长程记忆、跨平台能力与核心引擎强绑定时，无法按需扩展，资源占用与适配成本高
...
## 核心价值
1.  **rquickjs 插件体系（可选）**：基于进程内 `rquickjs` 的插件 VM，配合 `VmActor`、超时、中断预算、堆上限与 Hostcall 收口，兼顾灵活性与安全性
2.  **宿主内置原语与工具**：`read` / `write` / `edit` / `bash` 等作为宿主核心可信 API，全程权限管控、审计可追溯；内置 plan、todos、checkpoint、web 等工具按配置启用
3.  **安全自举全闭环（路线图）**：Agent 可基于自然语言完成「插件代码生成→沙箱内编译→热加载→错误修复」全流程（随插件系统解冻评估），全程隔离运行
4.  **稳定扩展契约**：宿主对插件暴露 ExtensionAPI、事件系统与工具注册规范，版本内行为一致，便于社区扩展与回归
5.  **极简分层架构**：核心引擎以「宿主可信层 + 可选插件 VM」为主；多 Agent、Skill、长程记忆等按 P0–P9 分档演进，核心轻量稳定
6.  **全平台原生支持**：基于 Rust 实现跨平台兼容，Windows / macOS / Linux 覆盖核心能力
7.  **精细化权限管控**：插件级与工作区级细粒度权限，默认最小权限，仅开放授权的原语、网络与文件访问

## 阶段性理念调整（2026-04-22 起）

项目经过一期 MVP 落地，已验证 Rust 宿主 + `rquickjs` 插件架构的可行性与插件扩展能力。当前阶段的战略调整：

- **先把单 Agent 做到极致**，再扩展到多 Agent、Skill、插件生态、UI 等周边能力；
- **插件系统冻结**：保留已完成的沙箱、4 原语、长生命周期 VM、ExtensionAPI 桥接层，不再新增插件特性；待单 Agent 体验成熟后再第二轮投资；
- **新路线图使用 P0-P9 十档**：P0-P9 不再是「紧急度」，而是**执行编排顺序**（上一档完成后再投入下一档），具体见下文。

## 当前状态（001-mvp 已关闭）
一期核心交付已完成并冻结：Rust 宿主核心、`rquickjs` 插件 VM、4 原语执行引擎、ExtensionAPI 桥接层、异步 Hostcall、长生命周期 VM（VM Actor）、CLI 对话模式、Agent Loop 三层循环、基础审计日志。`001-mvp` 时期归档文档已从仓库移除；若需溯源可查 **Git 历史**（曾位于 `docs/openspec/specs/archive/001-mvp/`）。

## 当前迭代：002-single-agent-complete（单 Agent 完善期）
完成 P0 + P1 两档，目标：让单 Agent 在 macOS/Linux 上的基础体验、状态管理、任务循环达到「无 P0 bug、可稳定长时间运行」。执行看板：[`docs/agents/TASK_BOARD_002/README.md`](../agents/TASK_BOARD_002/README.md)。
...
3. **Skill 系统**（调度、工作流、内置 Skill）—— P2；其中 v1 运行时技能（发现 / 披露 / `load_skill` / `/skill`）已在 `T2-P1-014` 提前落地，剩余工作流 / 内置资产仍在 P2；
4. **记忆系统 / USER.md / MEMORY.md**—— P3；
5. **自进化 / 学习回路**—— P4；
6. **跨平台（Android、openclaw 兼容等）**—— P7；
...
| **P2** Skill 系统 | 在已落地的 v1 运行时技能基础上继续补齐调度器、工作流引擎、内置高频 Skill | 新建 `src/core/skill/` | [plugin_skills_first_principles_pi_rust_wasm.md](../../docs/reports/plugin_skills_first_principles_pi_rust_wasm.md) §4-5、T-114/T-115 |
...
| **P7** 跨平台 | Android、openclaw 兼容与平台适配 | `scripts/`、`build.rs` | T-104/T-105/T-106/T-107 |
...
## 长期愿景
打造一款轻量、安全、可扩展的 AI Agent 运行时：从单 Agent 做到极致，让用户通过自然语言完成复杂任务，产出可审查的结果（日志、diff、测试、review）；再扩展到多 Agent 协作、Skill 复用、插件生态与跨平台，兼顾开放扩展与系统安全，推动 AI Agent 安全、可控地落地。
