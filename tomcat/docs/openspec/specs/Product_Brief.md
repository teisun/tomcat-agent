...
## 项目定位
tomcat 是一款基于 Rust + WasmEdge 构建的轻量、高安全、可自进化的 AI Agent 核心运行时。宿主侧提供会话、多模型 LLM、Agent 循环与内置工具；可选通过 WasmEdge 内置 QuickJS 与 Node.js 兼容层运行沙箱插件，打造「人人可扩展、安全可管控」的 AI Agent 底层引擎。
...
## 核心价值
1.  **WasmEdge+QuickJS 沙箱插件体系（可选）**：基于 WasmEdge 的 QuickJS 运行时，每个插件独立 Wasm 实例硬隔离，ExtensionAPI 与 Node.js 兼容层稳定，兼顾灵活性与安全性
...
5.  **极简分层架构**：核心引擎以「宿主可信层 + 可选 Wasm 沙箱」为主；多 Agent、Skill、长程记忆等按 P0–P9 分档演进，核心轻量稳定
6.  **全平台原生支持**：基于 Rust + WasmEdge 实现跨平台兼容，Windows / macOS / Linux 覆盖核心能力
7.  **精细化权限管控**：插件级与工作区级细粒度权限，默认最小权限，仅开放授权的原语、网络与文件访问

## 阶段性理念调整（2026-04-22 起）

项目经过一期 MVP 落地，已验证 Rust + WasmEdge + QuickJS 架构可行性与插件沙箱能力。当前阶段的战略调整：
...
## 当前状态（001-mvp 已关闭）
一期核心交付已完成并冻结：Rust 宿主核心、WasmEdge+QuickJS 沙箱、4 原语执行引擎、ExtensionAPI 桥接层、异步 Hostcall、长生命周期 VM（VM Actor）、CLI 对话模式、Agent Loop 三层循环、基础审计日志。`001-mvp` 时期归档文档已从仓库移除；若需溯源可查 **Git 历史**（曾位于 `docs/openspec/specs/archive/001-mvp/`）。
