...
2. **代理（按需）**：若直连上游不可用，在同一 `.env` 中配置 `HTTPS_PROXY`（及必要时 `HTTP_PROXY`）。`reqwest` 会读取当前进程环境；`llm_tests` / `openai_responses_integration_tests` 日志中都可见代理是否生效。
   - **私有 OpenAI 兼容网关实战备注（2026-06-20）**：当本机已接入内网时，`integration-openai-responses-wire` 更稳定的口径是**直连你的网关 host，不要再走本地 `127.0.0.1:7890` 代理**；否则 `responses_inline_image_describe_roundtrip` 可能返回 HTML `403 Forbidden`。建议在同一命令前显式加：
     `env -u HTTPS_PROXY -u https_proxy -u ALL_PROXY -u all_proxy -u HTTP_PROXY -u http_proxy NO_PROXY=<gateway-host> no_proxy=<gateway-host>`
