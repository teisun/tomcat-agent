...
**基准**：pi-mono `ExtensionAPI`（`packages/coding-agent/src/core/extensions/types.ts`）与 tomcat `assets/js/pi_bridge.js` + `HostApiDispatcher`。  
...
| wasmedge-quickjs `modules/` | 已落地 | `assets/modules/` 自 wasmedge-quickjs 同步；`instance_wasmedge.rs` 增加 `./modules` preopen；环境变量 `TOMCAT_QUICKJS_MODULES_PATH` 可覆盖 |
