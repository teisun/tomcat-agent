...
- **修复**：在 [`src/ext/host_binding.rs`](tomcat/src/ext/host_binding.rs) 中为 `HostRequest.params` 增加 `#[serde(default)]`，缺省时为空对象。
...
- **修复**：在 [`src/ext/dispatcher.rs`](tomcat/src/ext/dispatcher.rs) 的 `do_wait_for_event` 中，先克隆出 `Arc<Mutex<Receiver>>` 再立即释放 `get()` 的 Ref，仅在克隆出的 Arc 上 `lock()` 并 `recv()`，这样阻塞在 `recv()` 时不再持有 DashMap 的 shard 锁，`cleanup_instance` 可正常执行 `remove`、drop sender，使 `recv()` 返回 `Err` 并返回 `__shutdown`。
