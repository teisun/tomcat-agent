...
5. `tomcat init` 生成 `config.toml`，再手动编辑填入 API Key
6. `export OPENAI_API_KEY=...`
7. 手动复制 `wasmedge_quickjs.wasm` 到 `~/.tomcat/wasm/`
8. 确保 `pi_bridge.js` 可被发现（或设置 `PI_BRIDGE_JS_PATH`）
9. 确保 `assets/modules/` 可被发现（或设置 `TOMCAT_QUICKJS_MODULES_PATH`）

**目标**：发布预编译二进制后，用户只需 `tomcat init` 一条命令即可完成所有初始化。

---

## 2. 目标工作目录结构

与 [工作目录结构](../../docs/architecture/directory-structure.md) 一致（总控设计另见 [work-dir-and-data-layout](../../docs/architecture/work-dir-and-data-layout.md)）。本节强调与初始化、内嵌资源相关的布局；根目录承载主配置，全局运行时文件集中在 `assets/`。
...
└── assets/                         # 全局资源目录（内嵌释放与敏感环境变量）
    ├── .env                        # 敏感配置（API Key 等），tomcat init 自动生成
    ├── wasm/                       # 全局 Wasm 运行时引擎
...
| 现有路径 | 新路径 | 说明 |
|---------|--------|------|
| `~/.tomcat/` | `~/.tomcat/` | work_dir 根目录更名 |
| `config.toml` | `~/.tomcat/tomcat.config.toml` | 主配置在 work_dir 根目录，不再放在 `assets/` |
| `agents/default/sessions/` | `agents/<agentId>/sessions/` | MVP 主 agentId 为 `main`（与 `[agent]` / 默认配置一致） |
| `wasm/` | `assets/wasm/` | 移入 `assets/` |
| — | `assets/modules/` | 新增，内嵌 JS 模块的释放目标 |
| — | `assets/.env` | 新增，API Key 等敏感配置（与根目录 `tomcat.config.toml` 并列层级，路径为 `assets/.env`） |
...
- 修改 `resolve_quickjs_modules_dir`（当前位于 `src/ext/instance_wasmedge.rs`）：优先使用 `{work_dir}/assets/modules/`，不再依赖 `CARGO_MANIFEST_DIR` 或 `TOMCAT_QUICKJS_MODULES_PATH` 环境变量
...
- 在 `{work_dir}/assets/.versions.json` 中记录各资源的版本与摘要，供 `tomcat doctor` 读取展示
...
## 5. Phase 3：增强 `tomcat init` 交互式向导
...
### 5.2 `tomcat init` 幂等性与旧目录迁移

**幂等性**：`tomcat init` 可多次安全执行。
- 若 `tomcat.config.toml` 已存在，提示 `配置已存在，是否覆盖？[y/N]`；默认 N 保留
...
**旧目录迁移**：首次执行 `tomcat init` 时检测 `~/.tomcat/` 是否存在：
- 若存在 → 提示 `检测到旧工作目录 ~/.tomcat/，是否迁移到 ~/.tomcat/？[Y/n]`
- 迁移逻辑：`cp -r ~/.tomcat/* ~/.tomcat/` → 验证关键文件完整 → 重命名 `~/.tomcat/` 为 `~/.tomcat.bak/`
- 不自动删除旧目录，留 `.bak` 供用户确认后手动清理

### 5.3 `.env` 处理策略

**位置**：`{work_dir}/assets/.env`。主配置 `tomcat.config.toml` 位于 `{work_dir}` 根目录，与 `assets/` 并列；敏感项仅写入 `assets/.env`。

**生成逻辑**：
1. `tomcat init` 时自动生成 `.env` 模板
...
**文件权限**：`.env` 存放 API Key 等敏感信息，创建时设置 `0600`（仅属主可读写）。Linux/macOS 通过 `std::os::unix::fs::PermissionsExt` 设置；`tomcat doctor` 增加权限检查，若权限过宽则发出警告。

**模板内容**：

```env
# Tomcat / LLM API Key 配置
# 请填入你的 API Key 后保存
OPENAI_API_KEY=
...
### 5.5 `tomcat doctor` 增强

扩展检查项，发现问题时给出可执行的修复命令：

```
tomcat doctor
  ✓ 配置文件: ~/.tomcat/tomcat.config.toml
  ✓ WasmEdge 运行时: 0.13.5 (静态链接)
  ✓ QuickJS WASM: ~/.tomcat/assets/wasm/wasmedge_quickjs.wasm (3.3MB, sha256: abc...)
  ✓ Node 兼容模块: ~/.tomcat/assets/modules/ (80 files, sha256: def...)
  ✓ .env 权限: 0600
  ✗ LLM API Key: 未设置
    → 运行 `tomcat init` 或编辑 ~/.tomcat/assets/.env
```
...
| `docs/architecture/work-dir-and-data-layout.md` | 同步新目录布局 |
| `docs/architecture/directory-structure.md` | 与本文档第 2 节保持一致 |
...
| **环境变量降级** | 内嵌资源就绪后，`PI_BRIDGE_JS_PATH`、`TOMCAT_QUICKJS_MODULES_PATH`、`WASMEDGE_QUICKJS_PATH` 等环境变量仍作为**覆盖项**保留，便于开发调试；优先级：环境变量 > `{work_dir}/assets/` > 内嵌默认 |
...
- **Homebrew**: `brew install tomcat`（若发布 formula；单二进制，内含所有资源）
- **curl 安装脚本**: `curl -sSf https://example.tomcat.dev/install | sh`（占位 URL，以实际发布为准）
- **GitHub Releases**: 提供 macOS/Linux 预编译二进制

用户体验：

```bash
brew install tomcat    # 或下载二进制（若上游提供 formula 则为例示）
tomcat init            # 一条命令完成初始化
tomcat chat            # 开始使用
```
...
- [plugin-system-overview](../../docs/architecture/plugin-system-overview.md) — 插件系统概览
