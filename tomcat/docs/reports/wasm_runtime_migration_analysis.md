...
与 tomcat 当前架构存在根本性冲突：

| 维度 | tomcat 当前架构 | Javy 的模式 |
|------|----------------------|-------------|
...
| **net (TCP)** | `wasi_net` — TCP Server/Client/TLS | 无 | 无 | **不需要** — tomcat 不使用 TCP，网络通过 `__pi_host_call` 代理 |
...
| `setTimeout`/`clearTimeout` | 无法在 Command 模式下真正异步 | **无需** | 当前 tomcat 通过 `pi_main_loop.js` 的轮询机制实现 setTimeout，不依赖 C 层 timer |
...
不需要补齐：
  ├─ wasi_net (TCP/TLS) — tomcat 不直接使用，网络通过 host call 代理
...
| 耦合点 | 当前代码 | 问题 |
|--------|----------|------|
| import 具体类型 | `use tomcat::WasmEngine;` | 绑定 WasmEdge 后端具体 struct |
...
use tomcat::{create_runtime, WasmRuntime, WasmVm, WasmEngineConfig};
