...
> 范围：tomcat 现状分析 + pi-mono / openclaw 设计对照 + 业界方案 + **最终设计决策**
...
- [1. tomcat 现状与问题](#1-tomcat-现状与问题)
- [2. 参考项目策略](#2-参考项目策略)
- [3. 关键概念与数学模型](#3-关键概念与数学模型)
- [4. 三项目对比](#4-三项目对比)
- [5. 上下文语义完整性](#5-上下文语义完整性)
- [6. 最终设计决策（tomcat）](#6-最终设计决策tomcat)
...
| 策略 | 优点 | 缺点 | 适合场景 |
|------|------|------|----------|
| **tomcat 当前** | 最简单、可预测 | 不感知 token；大文件溢出；丢失早期上下文 | 原型阶段 |
...
> **结论**：tomcat 是编程 agent，应采用 **pi-mono 思路**分阶段实施。
...
> 日期：2026-03-30
> 状态：已实现（TASK-17 feature/context-management）
...
```
context_window        由模型固有参数决定（如  GPT-5.4 = 400,000）
max_output_tokens     模型单轮最大输出（如 GPT-5.4 = 128,000）
