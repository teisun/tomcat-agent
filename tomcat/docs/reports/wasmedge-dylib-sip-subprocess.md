...
| 场景 | 结果 |
|------|------|
| 终端直接运行 `tomcat chat` | 正常启动 ✓ |
| `tomcat chat` 内 LLM 调用 `execute_bash` 执行 `tomcat --help` | `dyld: Library not loaded: @rpath/libwasmedge.0.dylib` ✗ |
...
当前 `tomcat` 二进制**无 LC_RPATH**（可用 `otool -l tomcat | grep -A2 LC_RPATH` 验证为空），完全依赖 `DYLD_LIBRARY_PATH` 环境变量。
...
用户的 `tomcat` 二进制位于 `~/.cargo/bin/` 或项目 `target/` 下——**不在 SIP 保护路径**。从用户 shell 直接启动 `tomcat` 时，macOS 不剥离环境变量。
...
# 验证 rpath 绕过 SIP
cp target/release/tomcat /tmp/tomcat_copy
install_name_tool -add_rpath ~/.wasmedge/lib /tmp/tomcat_copy
/bin/sh -c '/tmp/tomcat_copy --help'
...
```
~/.tomcat-distribution/            ← 示例：与数据目录 ~/.tomcat 可分离的安装前缀
  bin/tomcat                      ← 二进制
...
|------|-----------|-------------|------|
| alice | /Users/alice/.tomcat-distribution/bin/tomcat | /Users/alice/.tomcat-distribution/lib/ | ✓ |
| bob | /Users/bob/.tomcat-distribution/bin/tomcat | /Users/bob/.tomcat-distribution/lib/ | ✓ |
| 系统级 | /usr/local/bin/tomcat | /usr/local/lib/ | ✓ |
...
**原理**：`tomcat init` 检测或安装 WasmEdge，将库的绝对路径写入配置文件。`execute_bash` 在构造 sh 命令时，从配置读取路径并包装为 `export DYLD_LIBRARY_PATH=...;` 前缀。sh 内部设的环境变量不受 SIP 影响（SIP 只在 exec 入口剥离外部传入的值）。
...
tomcat init
  ...
  ④ 将路径写入 tomcat.config.toml：wasmedge_lib_dir = "/Users/X/.wasmedge/lib"
...
Command::new("sh") → execve /bin/sh → SIP 剥离外部 DYLD（无所谓）
  ...
  → sh fork+exec tomcat（非 SIP 路径）→ macOS 不剥离 → tomcat 拿到变量 → 找到库 ✓
```
