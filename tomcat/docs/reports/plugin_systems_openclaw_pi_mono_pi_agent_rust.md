...
**落盘路径**：`tomcat/docs/reports/plugin_systems_openclaw_pi_mono_pi_agent_rust.md`
...
**另外**：本仓库里的 **`tomcat`** 不是第四套「并列产品」，只拿来当**旁白**（例如 hostcall、Wasm 隔间），帮助理解「为什么 pi_agent_rust 要收得那么紧」。
...
- **pi-mono 圈的 Extension**：在 `package.json` 里用 `pi.extensions` 指一个或多个入口文件；mono 里用一套 **ExtensionAPI**；Rust 里跑完会变成 **`RegisterPayload`** 那张总表。  
...
### 2.2 多入口包 `pi.extensions`（pi-mono 系专有问题）
...
### 5.3 旁证：tomcat 的 hostcall 模型（帮助理解 pi_agent_rust 类「窄宿主」）
...
**证据来源**：`tomcat/openspec/.../phase2-long-lived-vm.md` 明确以 **pi-mono 生态插件** 为例，说明 `git-checkpoint`、`todo`、`plan-mode`、`ssh`、定时器等依赖 **跨事件/跨 tool 调用** 的内存状态；并指出 **短生命周期 VM**「每次新建 VM、全局变量清零」的根本限制。
...
| 代价 3 | **与 pi-mono 扩展模型不 trivial 互移植**：清单格式、Gateway 装配、Skills 市场（ClawdHub）与 **`pi.extensions` 清单语义** **不等价**。 |
...
| 市场/分发 | **ClawdHub** 等技能市场属 **Skills 产品面**（`openclaw_docs`）；与 npm 插件分发关系未在源码展开 | 上游 npm/包管理 CLI（见 `tomcat` gap 报告） | 以本地路径 + `package_manager` 为主；**无**与 ClawdHub 对等的内置市场（源码未体现） |
...
| 同步钩子阻塞 | **未在插件导读量化** | pi-mono 插件可阻塞事件循环（引擎特性） | **未在已检出文档量化**；WASM/actor 方案见 tomcat openspec（**旁证**） |
...
| `tomcat/src/ext/README.md`（旁证） |
| `tomcat/docs/architecture/plugin-system/host-call-protocol.md`（旁证） |
| `tomcat/docs/architecture/plugin-system/phase2-long-lived-vm.md`（旁证） |
| `tomcat/docs/reports/pi_mono_gap_analysis.md` §3（旁证） |
