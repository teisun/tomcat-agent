...
> 日期：2026-03-29
> 范围：tomcat 现状分析 + pi-mono / openclaw 设计对照 + 业界方案 + 实施推荐
...
本章正文已迁至独立文档：[工具轮次与上下文管理深度分析](./context-management-deep-dive.md)。涵盖 tomcat 现状、reasoning loop 与 session 裁剪、pi-mono/openclaw 策略、数学模型与 `max_safe_rounds`、工具循环检测、上下文语义完整性、实现方案（A–E）多维度打分与分阶段推荐路径。
...
指 pi-mono 里的 `packages/ai`（npm 包名 `@mariozechner/pi-ai`），是统一 LLM provider 抽象层。它**不是** tomcat 的组件。
...
**对 tomcat 的影响**：`convert_to_llm_format` 需要按配置决定是否在 `Assistant` 消息中携带 `thinking_text`。建议增加 `strip_thinking_on_resend: bool` 配置（默认 true）。
