# 多会话协议对比：tomcat / pi-mono / OpenClaw

> 状态：**调研完成**
>
> 目的：梳理三个项目的会话存储与多会话机制，为 tomcat 实现「真·多会话」提供设计参考。

---

## 1. 摘要对比表

| 维度 | tomcat（当前 MVP） | pi-mono（coding-agent） | OpenClaw（Web UI） |
|------|------------------------|----------------------|-------------------|
...
## 2. tomcat 当前实现与本机磁盘状态

### 2.1 磁盘样例

`~/.tomcat/agents/main/sessions/sessions.json`（当前内容）：
...
```json
{
  "agent:main:main": {
    ...
    "sessionFile": "/Users/yankeben/.tomcat/agents/main/sessions/1774419565065_322f934f1b9d3bbc.jsonl",
...
pi-mono **不使用** `sessions.json` 索引文件。多会话 = 同一目录下多个 JSONL 文件（下列路径为 **pi-mono 上游**约定，与 tomcat 默认的 `~/.tomcat/agents/<agentId>/sessions/` 不同）：
...
tomcat 的 transcript 格式已与此兼容（v3 header、相同的 entry 类型枚举）。
...
OpenClaw 的「会话」主要指 Gateway 多通道聊天会话（与 coding-agent transcript 不同），但其 **「记住上次选中的会话」** 和 **LRU 缓存** 机制对 tomcat 有参考价值。
...
**对 tomcat 的启示**：可在 `sessions.json` 或独立 `state.json` 中持久化 `current_session_key`，启动时读取恢复，而非硬编码。
...
**对 tomcat 的启示**：tomcat 当前场景为 CLI 单进程，暂不需要 LRU 内存缓存。但若未来支持长驻进程（daemon / Web UI），可参考此模式限制内存中加载的会话数量。
...
- `tomcat chat`（默认）：`continueRecent` 按文件 mtime 选最近。
- `tomcat chat --resume`：列出目录中所有会话，用户选择。
...
- `tomcat chat`（默认）：读取 `current_key`，若无则选 `updated_at` 最大的。
...
| 项目 | 文件 | 说明 |
|------|------|------|
| tomcat | `src/core/session/store.rs` | `SessionStore` 类型、`DEFAULT_SESSION_KEY`、load/save |
| tomcat | `src/core/session/manager.rs` | `SessionManager`：CRUD、`current_session_key()`、transcript 读写 |
| tomcat | `src/core/session/transcript.rs` | `SessionHeader`、`TranscriptEntry`、append/read |
| tomcat | `src/api/cli.rs` | `run_session`：`new`/`list`/`switch`/`delete`/`archive`/`search` |
| tomcat | `src/api/chat.rs` | `ensure_session`、`chat_loop` |
| tomcat | `src/core/session/README.md` | 会话管理与 CLI 技术文档 |
