# tomcat 与 pi-mono 差距分析报告

**日期**：2026-03-22  
**基准**：pi-mono（TypeScript Monorepo，上游参考实现） vs tomcat（Rust + WasmEdge 重写）  
**方法**：对照 pi-mono 文档快照（`pi-mono_docs/`）、tomcat 源码与 `TASK_BOARD.md` 当前状态，按 12 个维度逐项分析。
...
- **pi-mono 侧 npm 扩展包具备完整系统权限**，用户需审源码
...
聚焦 P1 缺口，使 tomcat 在核心交互体验上接近 pi-mono。
...
6. **Skills / Prompt Templates**：从 `~/.tomcat/skills/` 加载技能文件；`--prompt-template` 参数
